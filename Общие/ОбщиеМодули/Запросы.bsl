...
Функция ДобавитьПолеСхемыЗапроса(Расположение, Выражение, Колонка = Неопределено) Экспорт
	...
	ВыражениеСхемы = ОператорСхемы.ВыбираемыеПоля.Добавить(ИмяПоля, ТипПоля);
	Если ЗначениеЗаполнено(Псевдоним) Тогда
		ВыражениеСхемы.Псевдоним = Псевдоним;
	КонецЕсли;
	
	Возврат ВыражениеСхемы;

КонецФункции // ДобавитьПолеСхемыЗапроса()
