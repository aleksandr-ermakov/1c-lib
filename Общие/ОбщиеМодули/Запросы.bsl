...
// Заполняет доступные поля описания временной таблицы по существующей в менеджере таблице.
// 
// Параметры:
...
//
Процедура ЗаполнитьОписаниеВременнойТаблицыПоТаблице(Описание, Таблица) Экспорт
	...
	Описание.ДоступныеПоля.Очистить();
	...
	Если ТипЗнч(Таблица) = Тип("ВременнаяТаблицаЗапроса") 
		...
		Для каждого Колонка Из Таблица.Колонки Цикл
			...
			Если ТипТолькоОдин Тогда
				Описание.ДоступныеПоля.Добавить(Колонка.Имя, Типы[0]);
			Иначе 	 
				Описание.ДоступныеПоля.Добавить(Колонка.Имя);
			КонецЕсли;
		КонецЦикла; 
