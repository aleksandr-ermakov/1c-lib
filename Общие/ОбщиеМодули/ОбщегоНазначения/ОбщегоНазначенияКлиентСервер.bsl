...
//
// #СДЕЛАТЬ А вообще нужна эта функция?
Функция ПолучитьПоляДанныхДереваФормыИерархическиРекурсивно(
	Узел, 
	Поля = "Идентификатор, ИдентификаторСтроки", 
	ИмяКоллекцииВложенныхСтрок = "Строки")
	...
	Для каждого Ветвь Из Узел.ПолучитьЭлементы() Цикл
		ПоляДанныхВетви = ПолучитьПоляДанныхДереваФормыИерархическиРекурсивно(Ветвь, Поля, ИмяКоллекцииВложенныхСтрок);
		СтруктураДанных[ИмяКоллекцииВложенныхСтрок].Добавить(ПоляДанныхВетви);
	КонецЦикла;
...
//
// Примеры:
//	МассивПоЗначениям(1, 2, 3);	// {1, 2, 3}
//	МассивПоЗначениям(1, Null, 3, Null); // {1, Null, 3} // Последний Null всегда отбрасывается
//
Функция МассивПоЗначениям(
	Элемент0 = Null,
	Элемент1 = Null,
	Элемент2 = Null,
	Элемент3 = Null,
...
КонецФункции // МассивПоЗначениям
...
//	Массив из Строка
//
Функция МассивИзСтроки(ИсходнаяСтрока) Экспорт
...
Функция РимскоеЧисло(Число, Знач Форма = Истина) Экспорт
	...
	ЭтоВалиднаяФорма = (ТипЗнч(Форма) = Тип("Число")) И (1 <= Форма И Форма <= 4) И (Число = Цел(Число))
	...
	Размах = 2 + Форма; // Разрядность вычитания младшей цифры из старшей. 2 означает что из L можно вычесть X и V.

	Вычитаемые = Новый СписокЗначений;
	МаксИндексРазряда = Разряды.Количество() - 1;
	Для каждого Разряд Из Разряды Цикл
		ИндексРазряда = Разряды.Индекс(Разряд);
		Вычитаемые.Добавить(Разряд.Значение, Разряд.Представление);
		Для МинусОтступ = - Размах По - 1 Цикл
			Отступ = - МинусОтступ;
			ИндексВычитаемого = ИндексРазряда + Отступ;
			Если ИндексВычитаемого > МаксИндексРазряда Тогда
				Продолжить;
			КонецЕсли;
			Вычитаемое				 = Разряды[ИндексВычитаемого];
			ВычитаемоеЗначение		 = Разряд.Значение - Вычитаемое.Значение;			// 9
			ВычитаемоеПредставление	 = Вычитаемое.Представление + Разряд.Представление;	// IX
			Если Разряды.НайтиПоЗначению(ВычитаемоеЗначение) <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Вычитаемые.Добавить(ВычитаемоеЗначение, ВычитаемоеПредставление);
		КонецЦикла;
	КонецЦикла;

	РимскоеЧисло = "";
	УменьшаемоеЗначение = Число;
	НачИндексВычитаемого = 0;
	Пока УменьшаемоеЗначение > 0 Цикл
		Для ИндексВычитаемого = НачИндексВычитаемого По Вычитаемые.Количество() - 1 Цикл
			Вычитаемое = Вычитаемые[ИндексВычитаемого];
			Если УменьшаемоеЗначение < Вычитаемое.Значение Тогда
				НачИндексВычитаемого = НачИндексВычитаемого + 1;
				Продолжить;
			Иначе
				РимскоеЧисло = РимскоеЧисло + Вычитаемое.Представление;
				УменьшаемоеЗначение = УменьшаемоеЗначение - Вычитаемое.Значение;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
