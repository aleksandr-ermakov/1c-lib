...
//	РежимКомпоновкиДанных	 - Булево	 - см. СхемаЗапроса.РежимКомпоновкиДанных.
//										 Позволяет избежать неверной интерпретации параметров компоновки данных в тексте запроса.
//
Процедура ЗаменитьЗапросПакета(
	...
	РежимКомпоновкиДанных = Ложь) Экспорт
	...
	ИсходнаяСхемаЗапроса.РежимКомпоновкиДанных = РежимКомпоновкиДанных;
...
// Источник добавляется без соединений к существующим источникам.
//
// Параметры:
//	Размещение			- ОператорВыбратьСхемыЗапроса | ИсточникиСхемыЗапроса - Размещение, в которое будет добавлен источник
//	ДоступнаяТаблица	- Строка, ДоступнаяТаблицаСхемыЗапроса, ДоступнаяВложеннаяТаблицаСхемыЗапроса - Добавляемая таблица
//	Псевдоним			- Строка - Псевдоним добавляемого источника
//
Функция ДобавитьИсточникСхемыЗапроса(Знач Размещение, Знач ДоступнаяТаблица, Знач Псевдоним = Неопределено) Экспорт

	Если ТипЗнч(Размещение) = Тип("ОператорВыбратьСхемыЗапроса") Тогда
		ИсточникиСхемы = Размещение.Источники;
	ИначеЕсли ТипЗнч(Размещение) = Тип("ИсточникиСхемыЗапроса") Тогда
		ИсточникиСхемы = Размещение;
	Иначе
		ВызватьИсключение "Параметр Размещение: Неожиданный тип.";
	КонецЕсли;

	НовыйИсточник = ИсточникиСхемы.Добавить(ДоступнаяТаблица, Псевдоним);
	ИзолироватьИсточникСхемыЗапроса(ИсточникСхемы, НовыйИсточник);

	Возврат НовыйИсточник;

Конецфункции // ДобавитьИсточникСхемыЗапроса()
...
// Устанавливает псевдоним колонке запроса схемы.
//
// Параметры:
//  ЗапросСхемы            - ЗапросВыбораСхемыЗапроса - Запрос схемы, в котором изменяется псевдоним колонки.
//  ВыражениеИлиПсевдоним  - ВыражениеСхемыЗапроса, Строка - Выражение колонки или ее текущий псевдоним.
//  Псевдоним              - Строка - Новый псевдоним колонки.
//
Процедура УстановитьПсевдонимКолонкеСхемыЗапроса(ЗапросСхемы, ВыражениеИлиПсевдоним, Псевдоним) Экспорт

	Колонка = ЗапросСхемы.Колонки.Найти(ВыражениеИлиПсевдоним);
	Если Колонка = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Не удалось найти колонку с выражением %1 в запросе схемы", Выражение);
	КонецЕсли;
	Колонка.Псевдоним = Псевдоним;

КонецПроцедуры // УстановитьПсевдонимКолонкеСхемыЗапроса()
	...
	Для каждого ПрисоединяемыйЗапрос Из СхемаЗапросаПрисоединяемая.ПакетЗапросов Цикл
		...
		Иначе	
			...
			Для каждого УдаляемаяКолонка Из УдаляемыеКолонки Цикл
				ЗапросСхемыИсточник.Колонки.Удалить(ЗапросСхемыИсточник.Колонки.Индекс(УдаляемаяКолонка));
			КонецЦикла; 
...
// Заполняет доступные поля описания временной таблицы по существующей таблице.
//	Перед заполнением коллекция доступных полей не очищается, чтобы не сломать структуру существующего запроса.
//	Не перезаполняет уже существующие доступные поля.
// 
// Параметры:
//	Описание			 - ОписаниеВременнойТаблицыСхемыЗапроса
//	Таблица				 - ВременнаяТаблицаЗапроса	 - Временная таблица внутри менеджера временных таблиц.
//						 - ТаблицаЗначений			 - Таблица значений как источник полей. 
//	ОчищатьОтсутствующие - Булево - Если Истина, доступные поля описания временной таблицы,
//									которых нет в Таблице, будут удалены.
//
Процедура ЗаполнитьОписаниеВременнойТаблицыПоТаблице(Описание, Таблица, ОчищатьОтсутствующие = Ложь) Экспорт
	
	Если ТипЗнч(Описание) <> Тип("ОписаниеВременнойТаблицыСхемыЗапроса") Тогда
		ВызватьИсключение "Параметр Описание: Ожидается тип ОписаниеВременнойТаблицыСхемыЗапроса.";
	КонецЕсли;

	ДоступныеПоля = Описание.ДоступныеПоля;

	Если ТипЗнч(Таблица) = Тип("ВременнаяТаблицаЗапроса") 
		Или ТипЗнч(Таблица) = Тип("ТаблицаЗначений") Тогда
		
		Если ОчищатьОтсутствующие Тогда     
			УдаляемыеПоля = Новый Массив;
			Для каждого ДоступноеПоле Из ДоступныеПоля Цикл
				Если Таблица.Колонки.Найти(ДоступноеПоле.Имя) = Неопределено Тогда
					УдаляемыеПоля.Добавить(ДоступноеПоле);
				КонецЕсли;
			КонецЦикла;                     
			Для каждого ДоступноеПоле Из УдаляемыеПоля Цикл
				ДоступныеПоля.Удалить(ДоступныеПоля.Индекс(ДоступноеПоле));
			КонецЦикла; 
		КонецЕсли;

		Для каждого Колонка Из Таблица.Колонки Цикл
			Типы = Колонка.ТипЗначения.Типы();
			ТипТолькоОдин = (Типы.Количество() = 1);
			ДоступноеПоле = ДоступныеПоля.Найти(Колонка.Имя);
			Если ДоступноеПоле = Неопределено Тогда
				Если ТипТолькоОдин Тогда
					ДоступныеПоля.Добавить(Колонка.Имя, Типы[0]);
				Иначе 	 
					ДоступныеПоля.Добавить(Колонка.Имя);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла; 

	Иначе
		ВызватьИсключение "Параметр Таблица: Ожидается тип ВременнаяТаблицаЗапроса или ТаблицаЗначений.";

	КонецЕсли;

КонецПроцедуры

// Убирает соединения между источников и другими таблицами.
//
// Параметры: 
//	Размещение - ОператорВыбратьСхемыЗапроса, ИсточникиСхемыЗапроса - Оператор запроса или коллекция источников,
//																		где нужно изолировать источник.
//	Источник - ИсточникСхемыЗапроса - Источник, который нужно изолировать.
//	
Процедура ИзолироватьИсточникСхемыЗапроса(Размещение, Источник) Экспорт
	...
	Источник.Соединения.Очистить(); // От источника к другим таблицам
	
	// От других таблиц к источнику:
	Для каждого ТекущийИсточник Из ИсточникиСхемы Цикл
		Соединения = ТекущийИсточник.Соединения;
		Для каждого Соединение Из Соединения Цикл
			Если Соединение.Источник = Источник Тогда
				ТекущийИсточник.Соединения.Удалить(Соединения.Индекс(Соединение));
				Прервать;
			КонецЕсли;
		КонецЦикла; 
	КонецЦикла;
...
// Получает параметр виртуальной таблицы по его имени, как оно указано в конструкторе запроса, или индексу.
//
// Параметры:
//  ТаблицаСхемы - ТаблицаСхемыЗапроса	 - Таблица, параметры которой получаются.
//  Параметр	 - Строка				 - Имя искомого параметра. См. ИменаПараметровВиртуальнойТаблицыСхемыЗапроса() 
//				 - Число				 - Индекс параметра.
// 
// Возвращаемое значение:
//  - ПараметрТаблицыСхемыЗапроса - Найденный параметр. 
//	- Неопределено - Если параметр не найден.
//
Функция ПараметрВиртуальнойТаблицыСхемыЗапроса(ТаблицаСхемы, Параметр) Экспорт
	
	Если ТипЗнч(Параметр) = Тип("Число") Тогда
		
		ИндексПараметра = Параметр;

	ИначеЕсли ТипЗнч(Параметр) = Тип("Строка") Тогда

		ИменаПараметров = ИменаПараметровВиртуальнойТаблицыСхемыЗапроса(ТаблицаСхемы);
		...
		ИндексПараметра = ИндексыПараметровВРег[ВРег(Параметр)];

		Если ИндексПараметра = Неопределено Тогда
			ВызватьИсключение "ТаблицаСхемы не содержит праметра и именем " + Параметр;
		КонецЕсли; 

	Иначе

		ВызватьИсключение "Параметр Параметр: Ожидается тип Число, Строка";
...
// Получает первый запрос выбора схемы запроса из схемы запроса.
//
// Параметры:
//  Размещение - СхемаЗапроса, ПакетЗапросовСхемыЗапроса  - Коллекция запросов или схема запроса, 
//															в которой производится поиск.
//
// Возвращаемое значение:
//  ЗапросВыбораСхемыЗапроса - Первый запрос выбора.
//  Неопределено - Если запрос не найден.
//
Функция ПервыйЗапросВыбораСхемыЗапроса(Размещение) Экспорт

	Если ТипЗнч(Размещение) = Тип("ПакетЗапросовСхемыЗапроса") Тогда
		Для каждого ЗапросСхемы Из Размещение Цикл
			Если ТипЗнч(ЗапросСхемы) = Тип("ЗапросВыбораСхемыЗапроса") Тогда
				Возврат ЗапросСхемы;
			КонецЕсли;
		КонецЦикла;
		Возврат Неопределено;
	ИначеЕсли ТипЗнч(Размещение) = Тип("СхемаЗапроса") Тогда
		Возврат ПервыйЗапросВыбораСхемыЗапроса(Размещение.ПакетЗапросов);
	Иначе
		ВызватьИсключение "Параметр Размещение: Неожиданный тип.";
	КонецЕсли;

КонецФункции // ПервыйЗапросВыбораСхемыЗапроса()

// Получает первый оператор выбора схемы запроса из схемы запроса.
//
// Параметры:
//  Размещение - СхемаЗапроса, ЗапросВыбораСхемыЗапроса, ОператорыСхемыЗапроса - Размещение,
//					из которого будет получен первый оператор выбора схемы запроса.
//
// Возвращаемое значение:
//  ОператорВыбратьСхемыЗапроса - Первый оператор выбора.
//  Неопределено - Если оператор не найден.
//
Функция ПервыйОператорВыбратьСхемыЗапроса(Размещение) Экспорт

	Если ТипЗнч(Размещение) = Тип(ОператорыСхемыЗапроса) Тогда
		Возврат Размещение[0];
	ИначеЕсли ТипЗнч(Размещение) = Тип("ЗапросВыбораСхемыЗапроса") Тогда
		Возврат Размещение.Операторы[0];
	ИначеЕсли ТипЗнч(Размещение) = Тип("СхемаЗапроса") Тогда
		ЗапросВыбора = ПервыйЗапросВыбораСхемыЗапроса(Размещение);
		Если ЗапросВыбора <> Неопределено Тогда
			Возврат ПервыйОператорВыбратьСхемыЗапроса(ЗапросВыбора);
		Иначе
			Возврат Неопределено;
		КонецЕсли;
	Иначе
...
КонецФункции // ПервыйОператорВыбратьСхемыЗапроса()

// Соединяет два источника схемы запроса с указанным условием соединения.
//
// Параметры:
//  ИсточникСлева	 - ИсточникСхемыЗапроса - Источник, к которому будет присоединён другой источник.
//  ИсточникСправа	 - ИсточникСхемыЗапроса - Источник, который будет присоединён к другому источнику.
//  Условие			 - ВыражениеСхемыЗапроса, Строка - Условие соединения источников.
//  ВидСоединения	 - ВидСоединенияСхемыЗапроса - Вид соединения источников.
//  					Если не указан, будет использовано внутреннее соединение.
//
// Возвращаемое значение:
//  СоединениеСхемыЗапроса - Созданное соединение.
//	Неопределено - Если соединение не было создано (например, оно уже существует).
//
Функция СоединитьИсточникиСхемыЗапроса(Знач ИсточникСлева, Знач ИсточникСправа, Условие, Знач ВидСоединения = Неопределено) Экспорт

	Если ТипЗнч(ИсточникСлева) <> Тип("ИсточникСхемыЗапроса") Тогда
		ВызватьИсключение "Параметр ИсточникСлева: Ожидается тип ИсточникСхемыЗапроса.";
	КонецЕсли;
	
	Если ТипЗнч(ИсточникСправа) <> Тип("ИсточникСхемыЗапроса") Тогда
		ВызватьИсключение "Параметр ИсточникСправа: Ожидается тип ИсточникСхемыЗапроса.";
	КонецЕсли;

	СоединениеДобавлено = ИсточникСлева.Соединения.Добавить(ИсточникСправа, Условие);
	Если Не СоединениеДобавлено Тогда
		Возврат Неопределено;
	КонецЕсли;

	Если ВидСоединения = Неопределено Тогда
		ВидСоединения = ВидСоединенияСхемыЗапроса.Внутреннее;
	КонецЕсли;

	НовоеСоединение = ИсточникСлева.Соединения[ИсточникСлева.Соединения.Количество() - 1];
	НовоеСоединение.Вид = ВидСоединения;
	
	Возврат НовоеСоединение;

КонецФункции // СоединитьИсточникиСхемыЗапроса()

// Устанавлиает параметр виртуальной таблицы по его имени, как оно указано в конструкторе запроса, или индексу.
//
// Параметры:
//  ТаблицаСхемы - ТаблицаСхемыЗапроса	 - Таблица, параметры которой получаются.
//  Параметр	 - Строка				 - Имя искомого параметра. См. ИменаПараметровВиртуальнойТаблицыСхемыЗапроса() 
//				 - Число				 - Индекс параметра.
//	Выражение	 - ВыражениеСхемыЗапроса, Строка - Выражение параметра.
// 
// Возвращаемое значение:
//  - ПараметрТаблицыСхемыЗапроса - Найденный параметр. 
//	- Неопределено - Если параметр не найден.
//
Процедура УстановитьПараметрВиртуальнойТаблицыСхемыЗапроса(ТаблицаСхемы, Параметр, Знач Выражение) Экспорт
	...
	ИначеЕсли ТипЗнч(Параметр) = Тип("Строка") Тогда
		...
		Если ИндексПараметра = Неопределено Тогда
			ВызватьИсключение "ТаблицаСхемы не содержит параметра и именем " + Параметр;
		КонецЕсли; 
	...
	Если ТипЗнч(Выражение) = Тип("ВыражениеСхемыЗапроса") Тогда
		ВыражениеСхемы = Выражение;
	ИначеЕсли ТипЗнч(Выражение) = Тип("Строка") Тогда
		ВыражениеСхемы = Новый ВыражениеСхемыЗапроса(Выражение);
	Иначе
		ВызватьИсключение "Параметр Выражение: Ожидается тип ВыражениеСхемыЗапроса, Строка.";
	КонецЕсли;

	ТаблицаСхемы.Параметры[ИндексПараметра].Выражение = ВыражениеСхемы;
	
КонецПроцедуры // ПараметрВиртуальнойТаблицыСхемыЗапроса()

// Получает имена параметров виртуальной таблицы.
//
// Параметры:
//  Таблица	 - ТаблицаСхемыЗапроса, Строка - Таблица схемы запроса или полное имя таблицы.
//
// Возвращаемое значение:
//   Массив из Строка   - Имена параметров в порядке их следования. 
//						Индекс имени в массиве соответствует индексу параметра.
//
// Имена параметров таблиц:
//
//		 Регистр сведений (срез первых и последних) (РегистрСведений.*.СрезПервых, РегистрСведений.*.СрезПоследних):
//			* Период
//			* Условие
//		
//		 Регистр накопления, остатки (РегистрНакопления.*.Остатки):
//			* Период
//			* Условие
//		
//		 Регистр накопления, обороты (РегистрНакопления.*.Обороты):
//			* НачалоПериода
//			* КонецПериода
//			* Периодичность
//			* Условие
//		
//		 Регистр накопления, остатки и обороты (РегистрНакопления.*.ОстаткиИОбороты):
//			* НачалоПериода
//			* КонецПериода
//			* Периодичность
//			* МетодДополнения
//			* Условие
//		
//		 Регистр бухгалтерии, остатки (РегистрБухгалтерии.*.Остатки):
//			* Период
//			* УсловиеСчета
//			* Субконто
//			* Условие
//		
//		 Регистр бухгалтерии, обороты (РегистрБухгалтерии.*.Обороты):
//			* НачалоПериода
//			* КонецПериода
//			* Периодичность
//			* УсловиеСчета
...
//		
//		 Регистр бухгалтерии, обороты Дт Кт (РегистрБухгалтерии.*.ОборотыДтКт):
//			* НачалоПериода
//			* КонецПериода
//			* Периодичность
//			* УсловиеСчетаДт
...
//			* Условие
//		
//		 Регистр бухгалтерии, остатки и обороты (РегистрБухгалтерии.*.ОстаткиИОбороты): 
//			* НачалоПериода
//			* КонецПериода
//			* Периодичность
//			* МетодДополнения
...
//			* Условие
//		
//		 Регистр бухгалтерии, движения с субконто (РегистрБухгалтерии.*.ДвиженияССубконто): 
//			* НачалоПериода
//			* КонецПериода
//			* Условие
//			* Упорядочивание
//			* Первые
//		
//		 Регистр расчетов, фактический период действия (РегистрРасчетов.*.ФактическийПериодДействия"):
//			* Условие
//		
//		 Регистр расчетов, данные графика (РегистрРасчетов.*.ДанныеГрафика"):
//			* Условие
//		
//		 Регистр расчета, таблица базовых данных (РегистрРасчетов.*.База):
//			* ИзмеренияОсновногоРегистра
//			* ИзмеренияБазовогоРегистра
//			* Разрезы
//			* Условие
//		
//		 Критерий отбора (КритерийОтбора.*):
//			* Значение
//		
//		 Задача, задачи по исполнителю (Задача.*.ЗадачиПоИсполнителю):
//			* Исполнитель
//			* Условие
//
Функция ИменаПараметровВиртуальнойТаблицыСхемыЗапроса(Таблица)

	Если ТипЗнч(Таблица) = Тип("ТаблицаСхемыЗапроса") Тогда
		ИмяТаблицы = Таблица.ИмяТаблицы;
	ИначеЕсли ТипЗнч(Таблица) = Тип("Строка") Тогда
		ИмяТаблицы = Таблица;
	Иначе
		ВызватьИсключение "Параметр Таблица: Ожидается тип ТаблицаСхемыЗапроса, Строка.";
	КонецЕсли;
	...
	ИначеЕсли СтрНачинаетсяС(ИмяТаблицы, "РегистрНакопления.") Тогда 
	...
	ИначеЕсли СтрНачинаетсяС(ИмяТаблицы, "РегистрБухгалтерии.") Тогда
		...
		ИначеЕсли СтрЗаканчиваетсяНа(ИмяТаблицы, ".Обороты") Тогда
			...
			Если ТипЗнч(Таблица) = Тип("ТаблицаСхемыЗапроса") Тогда
				ЕстьКорреспонденция = Таблица.Параметры.Количество() > ИменаПараметров.Количество();
			Иначе
				ИмяРегистра = СтрРазделить(Таблица, ".")[1];
				ЕстьКорреспонденция = Метаданные.РегистрыБухгалтерии[ИмяРегистра].Корреспонденция;
			КонецЕсли;
...
// Формирует текст запроса транзитивного замыкания элементов справочника или плана видов характеристик.
//
// Параметры:
//	ПолноеИмяОбъектаМетаданных	 - Строка - Имя справочника или плана видов характеристик в метаданных
...
//	Строка - Текст запроса, формирующий таблицу с полями:
...
//
Функция ТекстЗапросаТранзитивноеЗамыкание(ПолноеИмяОбъектаМетаданных, МаксимальнаяДлинаПути = Неопределено) Экспорт
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ВызватьИсключение СтрШаблон("Объект метаданных с именем %1 не найден.", ПолноеИмяОбъектаМетаданных);
	КонецЕсли;

	ЭтоСправочник = Метаданные.Справочники.Содержит(ОбъектМетаданных);
	ЭтоПланВидовХарактеристик = Не ЭтоСправочник И Метаданные.ПланыВидовХарактеристик.Содержит(ОбъектМетаданных);
	Если Не ЭтоСправочник И Не ЭтоПланВидовХарактеристик Тогда
		ВызватьИсключение СтрШаблон(
			"Объект метаданных %1: Ожидается Справочник, ПланВидовХарактеристик",
	...
	Если МаксимальнаяДлинаПути = Неопределено Тогда
		...
		Если ЭтоСправочник И ОбъектМетаданных.ОграничиватьКоличествоУровней Тогда
			МаксимальнаяДлинаПути = ОбъектМетаданных.КоличествоУровней;
		ИначеЕсли Не МетаСправочник.Иерархический Тогда
	...
	Пока МаксимальнаяДлинаЗамыканий < МаксимальнаяДлинаПути Цикл	// Рефрен
		ЧастиЗапроса.Добавить(
			СтрШаблон(
				Рефрен, 
				Формат(МаксимальнаяДлинаЗамыканий, "ЧГ=0"), 
...
//
// Параметры:
//	ИмяТаблицы				 - Строка	 - Имя таблицы с исходными данными. 
//											Например, "Справочник.ИерархическийСправочник".
//	ПолеРодитель			 - Строка	 - Поле, содержащее родителя. Например, "Родитель".
//	ПолеПотомок				 - Строка	 - Поле, содержащее собственный идентификатор строка. Например, "Ссылка".
//	ВыражениеПустоеЗначение	 - Строка	 - Выражение для проверки корневых родителей. 
//											Например, "ЗНАЧЕНИЕ(Справочник.ИерархическийСправочник.ПустаяСсылка)".
//	МаксимальнаяДлинаПути	 - Число	 - Максимальная длина определяемого пути между элементами.
//											Должна покрывать максимальную глубину иерархии.
//	ТаблицаДляПомещения		 - Строка	 - Имя таблицы для помещения результата.
//
// ВозвращаемоеЗначение:
//	Строка - Текст запроса, формирующий таблицу с полями:
//		* Предок	 - Произвольный	 - Элемент-предок.
//		* Потомок	 - Произвольный	 - Элемент-потомок.
//		* ДлинаДуги	 - Число			 - Количество шагов наследственности от предка до потомка. 
//										Для непосредственных родителей равна 1.
//
Функция ТекстЗапросаТранзитивноеЗамыкание(ИмяТаблицы, ПолеРодитель, ПолеПотомок, 
	ВыражениеПустоеЗначение = "NULL", МаксимальнаяДлинаПути = 64, ТаблицаДляПомещения = "") Экспорт

	// Методика: https://infostart.ru/public/158512/

	Пролог = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ &Родитель НачалоДуги, &Потомок КонецДуги, 1 ДлинаДуги 
	| ПОМЕСТИТЬ ЗамыканияДлины1 ИЗ &Таблица
	| ГДЕ &Родитель <> &ПустоеЗначение
	| ОБЪЕДИНИТЬ ВЫБРАТЬ &Потомок, &Потомок, 0 ИЗ &ИмяТаблицы ГДЕ &Потомок <> &ПустоеЗначение";
	...
	Эпилог = 
	"ВЫБРАТЬ НачалоДуги Предок, КонецДуги Потомок, ДлинаДуги 
	| ПОМЕСТИТЬ ТаблицаДляПомещения
	| ИЗ ЗамыканияДлины%1 ГДЕ НачалоДуги <> КонецДуги";

	ЧастиЗапроса = Новый Массив;
	Пролог = СтрЗаменить(Пролог, "&Таблица",		 ИмяТаблицы);
	Пролог = СтрЗаменить(Пролог, "&Родитель",		 ПолеРодитель);
	Пролог = СтрЗаменить(Пролог, "&Потомок",		 ПолеПотомок);
	Пролог = СтрЗаменить(Пролог, "&ПустоеЗначение",	 ВыражениеПустоеЗначение);
	ЧастиЗапроса.Добавить(Пролог);	// Пролог
	...
	Если ПустаяСтрока(ТаблицаДляПомещения) Тогда
		Эпилог = СтрЗаменить(Эпилог, "ПОМЕСТИТЬ ТаблицаДляПомещения", "");
	Иначе
		Эпилог = СтрЗаменить(Эпилог, "ПОМЕСТИТЬ ТаблицаДляПомещения", "ПОМЕСТИТЬ " + ТаблицаДляПомещения);
	КонецЕсли;
