...
Функция ЦветПоКоду(Знач КодЦвета) Экспорт
	...
	ИспользоватьСериализатор = СтрНачинаетсяС(КодЦвета, "#") И СтрДлина(КодЦвета) = 7;
	Если ИспользоватьСериализатор Тогда
		Цвет = СериализаторXDTO.XMLЗначение(Тип("Цвет"), КодЦвета);
		Возврат Цвет;
	КонецЕсли;

	Если СтрНачинаетсяС(КодЦвета, "#") Тогда
		КодЦвета = Сред(КодЦвета, 2);
	КонецЕсли;
	Если СтрДлина(КодЦвета) = 3 Тогда
		КодЦвета = Сред(КодЦвета, 1, 1) + Сред(КодЦвета, 1, 1)
				 + Сред(КодЦвета, 2, 1) + Сред(КодЦвета, 2, 1)
				 + Сред(КодЦвета, 3, 1) + Сред(КодЦвета, 3, 1);
	КонецЕсли;
	КодЦвета = Врег(КодЦвета);
	Если СтрДлина(КодЦвета) <> 6 Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	Разряды	 = "0123456789ABCDEF";
		
	Для НомерСимвола = 1 По СтрДлина(КодЦвета) Цикл
		Если Не СтрНайти(Разряды, Сред(КодЦвета, НомерСимвола, 1)) Тогда
			Возврат Неопределено;
		КонецЕсли;
	КонецЦикла;
	...
	Для Индекс = 0 По 2 Цикл
		ЧастьКода = Сред(КодЦвета, Индекс * 2 + 1, 2);
		ЦветЧислом	 = (СтрНайти(Разряды, Лев(ЧастьКода, 1)) - 1) * 16
					 + (СтрНайти(Разряды, Прав(ЧастьКода, 1)) - 1);
