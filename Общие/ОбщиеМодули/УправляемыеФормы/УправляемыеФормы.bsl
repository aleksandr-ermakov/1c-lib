
// Рассчитывает итоги динамическиого списка, показанного на форме.
//
// Параметры:
//  ТаблицаФормы - ТаблицаФормы	 - Таблица, отображающая данные динамического списка.
//  ПоляИтогов	 - Строка		 - Имена полей итогов, через запятую.
// 
// Возвращаемое значение:
//  Структура - Итоги по списку.
//
Функция ИтогиДинамическогоСписка(Знач ТаблицаФормы, Знач ПоляИтогов) Экспорт
	
	СхемаКомпоновкиДанных	 = ТаблицаФормы.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
	НастройкиКомпоновки		 = ТаблицаФормы.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();   
	ВозможностьИспользованияВнешнихФункций = Истина;
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(
		СхемаКомпоновкиДанных, НастройкиКомпоновки, , , 
		Тип("ГенераторМакетаКомпоновкиДанныхДляКоллекцииЗначений")
	);
	ПроцессорКомпоновки = Новый ПроцессорКомпоновкиДанных;
	ПроцессорКомпоновки.Инициализировать(МакетКомпоновки, , , ВозможностьИспользованияВнешнихФункций);
	
	РезультатКомпоновки = Новый ТаблицаЗначений;
	
	ПроцессорВывода = Новый ПроцессорВыводаРезультатаКомпоновкиДанныхВКоллекциюЗначений;
	ПроцессорВывода.УстановитьОбъект(РезультатКомпоновки);
	ПроцессорВывода.Вывести(ПроцессорКомпоновки);
	
	ПоляИтогов = СтрРазделить(ПоляИтогов, ", ", Ложь);
	Итоги = Новый Структура;
	Для каждого ПолеИтога Из ПоляИтогов Цикл
		Итоги.Вставить(ПолеИтога, РезультатКомпоновки.Итог(ПолеИтога));
	КонецЦикла;      
	
	Возврат Итоги;

КонецФункции // ИтогиДинамическогоСписка()

// Добавляет декорации формы, которые отображают легенду условного оформления формы.
//
// Параметры:
//	ГруппаЭлементовЛегенды	 - ГруппаФормы		 - Группа, куда будут помещены элементы легенды
//	ОбластьОформления		 - ПолеФормы, ГруппаФормы, ТаблицаФормы, УправляемаяФорма, ДополнениеЭлементаФормы - Фильтр по затрагиваемым элементам формы
//	Форма					 - УправляемаяФорма	 - Оформляемая форма.
//
Процедура СоздатьЛегендуУсловногоОформления(ГруппаЭлементовЛегенды, 
	ОбластьОформления = Неопределено, 
	Форма = Неопределено)
	
	//Форма = ЭтаФорма;	// Раскомментируйте строку для использвания в контексте формы

	ПрефиксГруппы		 = ГруппаЭлементовЛегенды.Имя;
	ПрефиксДекорации	 = "ДекорацияЛегенды";
	
	// Чистим существующие:
	Для каждого ЭлементФормы Из ГруппаЭлементовЛегенды.ПодчиненныеЭлементы Цикл
		Если СтрНайти(ЭлементФормы.Имя, ПрефиксГруппы + ПрефиксДекорации) = 1 Тогда
			Форма.Элементы.Удалить(ЭлементФормы);
		КонецЕсли; 
	КонецЦикла; 

	ОформляемыеЭлементыФормы = Новый Массив;
	ИменаОформляемыхЭлементовФормы = Новый Массив;
	Если ОбластьОформления <> Неопределено Тогда

		ОформляемыеЭлементыФормы = Новый Массив;
		ОформляемыеЭлементыФормы.Добавить(ОбластьОформления);
		Для каждого ОформляемыйЭлементФормы Из ОформляемыеЭлементыФормы Цикл
			Если ТипЗнч(ОформляемыйЭлементФормы) = Тип("ГруппаФормы")
				Или ТипЗнч(ОформляемыйЭлементФормы) = Тип("ТаблицаФормы") 
				Или ТипЗнч(ОформляемыйЭлементФормы) = Тип("ФормаКлиентскогоПриложения") 
				Или ТипЗнч(ОформляемыйЭлементФормы) = Тип("ДополнениеЭлементаФормы") Тогда
				
				Для каждого ПодчиненныйЭлемент Из ОформляемыйЭлементФормы.ПодчиненныеЭлементы Цикл
					ОформляемыеЭлементыФормы.Добавить(ПодчиненныйЭлемент);
				КонецЦикла; 
				
			КонецЕсли; 
		КонецЦикла; 
		
		Для каждого Элемент Из ОформляемыеЭлементыФормы Цикл
			ИменаОформляемыхЭлементовФормы.Добавить(Элемент.Имя);
		КонецЦикла; 
		
	КонецЕсли; 
	ФильтроватьПоЭлементамФормы = ЗначениеЗаполнено(ОформляемыеЭлементыФормы);
	
	ОтображаемыеПараметрыОформления = Новый Массив;
	ОтображаемыеПараметрыОформления.Добавить(Новый ПараметрКомпоновкиДанных("Шрифт"));
	ОтображаемыеПараметрыОформления.Добавить(Новый ПараметрКомпоновкиДанных("ЦветФона"));
	ОтображаемыеПараметрыОформления.Добавить(Новый ПараметрКомпоновкиДанных("ЦветТекста"));
	ОтображаемыеПараметрыОформления.Добавить(Новый ПараметрКомпоновкиДанных("ЦветРамки"));
	ОтображаемыеПараметрыОформления.Добавить(Новый ПараметрКомпоновкиДанных("ЦветРамки"));
	ОтображаемыеПараметрыОформления.Добавить(Новый ПараметрКомпоновкиДанных("Формат"));
	ОтображаемыеПараметрыОформления.Добавить(Новый ПараметрКомпоновкиДанных("Текст"));
	
	НомерДекорации = 0;
	Для каждого ЭлементУсловногоОформления Из Форма.УсловноеОформление.Элементы Цикл
		
		НомерДекорации	 = НомерДекорации + 1;
		ИмяЭлементаФормы = ПрефиксГруппы + ПрефиксДекорации + Формат(НомерДекорации, "ЧГ=");
		
		Если не ЭлементУсловногоОформления.Использование Тогда
			Продолжить;
		КонецЕсли; 
		
		Если ФильтроватьПоЭлементамФормы Тогда
			ЕстьЗатрагивыемыеЭлементы = Ложь;
			Для каждого ОформляемоеПоле Из ЭлементУсловногоОформления.Поля.Элементы Цикл
				Если ОформляемоеПоле.Использование 
					И ИменаОформляемыхЭлементовФормы.Найти(Строка(ОформляемоеПоле.Поле)) <> Неопределено Тогда
					ЕстьЗатрагивыемыеЭлементы = Истина;	
					Прервать;
				КонецЕсли; 
			КонецЦикла; 
			Если не ЕстьЗатрагивыемыеЭлементы Тогда
				Продолжить;
			КонецЕсли; 
		КонецЕсли; 
		
		ЭлементФормы = Неопределено;
		ПредставлениеОтбора = Строка(ЭлементУсловногоОформления.Отбор);
		СоставПредставленияФормата = Новый Массив;
		ПодсказкаФормат = "";
		ПодсказкаТекст = "";
		Для каждого ЗначениеПараметра Из ЭлементУсловногоОформления.Оформление.Элементы Цикл

			Если не ЗначениеПараметра.Использование Тогда Продолжить КонецЕсли; 
			Если ОтображаемыеПараметрыОформления.Найти(ЗначениеПараметра.Параметр) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Если ЭлементФормы = Неопределено Тогда
				ЭлементФормы = Форма.Элементы.Добавить(ИмяЭлементаФормы, Тип("ДекорацияФормы"), ГруппаЭлементовЛегенды);
				Если ЗначениеЗаполнено(ЭлементУсловногоОформления.Представление) Тогда
					ЭлементФормы.Заголовок = ЭлементУсловногоОформления.Представление;
				ИначеЕсли ЗначениеЗаполнено(ПредставлениеОтбора) Тогда
					ЭлементФормы.Заголовок = ПредставлениеОтбора;
				Иначе
					ЭлементФормы.Заголовок = "Безусловное оформление";
				КонецЕсли; 
			КонецЕсли; 
			// Оформление:
			
			Если ЗначениеПараметра.Параметр = Новый ПараметрКомпоновкиДанных("Шрифт") Тогда
				ЭлементФормы.Шрифт = ЗначениеПараметра.Значение;				
			КонецЕсли; 
			Если ЗначениеПараметра.Параметр = Новый ПараметрКомпоновкиДанных("ЦветФона") Тогда
				ЭлементФормы.ЦветФона = ЗначениеПараметра.Значение;				
			КонецЕсли; 
			Если ЗначениеПараметра.Параметр = Новый ПараметрКомпоновкиДанных("ЦветТекста") Тогда
				ЭлементФормы.ЦветТекста = ЗначениеПараметра.Значение;				
			КонецЕсли; 
			Если ЗначениеПараметра.Параметр = Новый ПараметрКомпоновкиДанных("ЦветРамки") Тогда
				ЭлементФормы.ЦветРамки = ЗначениеПараметра.Значение;				
			КонецЕсли; 
			
			Если ЗначениеПараметра.Параметр = Новый ПараметрКомпоновкиДанных("Текст") Тогда
				ПодсказкаТекст = ЗначениеПараметра.Значение;				
			КонецЕсли; 
			
		КонецЦикла;
		
		ПодсказкаОформление = Строка(ЭлементУсловногоОформления.Оформление);
		
		ЭлементыРасширеннойПодсказки = Новый СписокЗначений;
		ЭлементыРасширеннойПодсказки.Добавить(ПредставлениеОтбора,	 "Условие");
		ЭлементыРасширеннойПодсказки.Добавить(ПодсказкаОформление,	 "Оформление");
		ЭлементыРасширеннойПодсказки.Добавить(ПодсказкаФормат,		 "Формат");
		ЭлементыРасширеннойПодсказки.Добавить(ПодсказкаТекст,		 "Текст");
		Если ЭлементФормы <> Неопределено Тогда
			СоставРасширеннойПодсказки = Новый Массив;
			Для каждого ЭлементРасширеннойПодсказки Из ЭлементыРасширеннойПодсказки Цикл
				
				Если Не ЗначениеЗаполнено(ЭлементРасширеннойПодсказки.Значение) Тогда 
					Продолжить;
				КонецЕсли;
					
				Если ЗначениеЗаполнено(СоставРасширеннойПодсказки) Тогда 
					СоставРасширеннойПодсказки.Добавить(Символы.ПС + Символы.ПС);
				КонецЕсли; 
				СоставРасширеннойПодсказки.Добавить(
					Новый ФорматированнаяСтрока(
						ЭлементРасширеннойПодсказки.Представление + ":	", 
						Новый Шрифт(, , Истина)));
				СоставРасширеннойПодсказки.Добавить(Символы.ПС);
				СоставРасширеннойПодсказки.Добавить(ЭлементРасширеннойПодсказки.Значение);

			КонецЦикла; 
			
			ЭлементФормы.РасширеннаяПодсказка.Заголовок = Новый ФорматированнаяСтрока(СоставРасширеннойПодсказки);
			ЭлементФормы.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
			
		КонецЕсли; 
		
	КонецЦикла; 

КонецПроцедуры // СоздатьЛегендуУсловногоОформления()

// Находит реквизит формы по имени
//
// Параметры:
//  Форма		 - ФормаКлиентскогоПриложения	 - Форма, где ищем.
//  ПутьКДанным	 - Строка						 - Путь к данным
// 
// Возвращаемое значение:
//   РеквизитФормы, Неопределено  - Найденный реквизит.
//
Функция НайтиРеквизитФормы(Форма, ПутьКДанным) Экспорт

	Если Не ЗначениеЗаполнено(ПутьКДанным)
		Или ТипЗнч(ПутьКДанным) <> Тип("Строка") Тогда
		
		ВызватьИсключение "Параметр ПутьКДанным: Ожидается значение (Тип: Строка)";		
	
	КонецЕсли; 
	
	Если СтрНайти(ПутьКДанным, ".") Тогда
		
		// Поиск реквизитов ограничиваем путем
		Путь = СтрРазделить(ПутьКДанным, ".");
		ИмяРеквизита = Путь[Путь.ВГраница()];
		Путь.Удалить(Путь.ВГраница());
		Путь = СтрСоединить(Путь, ".");
		РеквизитыФормы = Форма.ПолучитьРеквизиты(Путь);
		
	Иначе
		
		ИмяРеквизита = ПутьКДанным;
		РеквизитыФормы = Форма.ПолучитьРеквизиты();
	
	КонецЕсли; 
	
	Для каждого РеквизитФормы Из РеквизитыФормы Цикл
		Если РеквизитФормы.Имя = ИмяРеквизита Тогда
			Возврат РеквизитФормы;
		КонецЕсли; 				
	КонецЦикла; 
	
	Возврат Неопределено;

КонецФункции // НайтиРеквизитФормы()

// #СДЕЛАТЬ Добавить выгрузку других типов коллекций
// Выгружает в таблицу значений коллекцию формы, 
// содержащую в качестве колонок вложенные таблицы.
//
// Параметры:
//	ДанныеФормыКоллекция - ДанныеФормыКоллекция - Выгружаемая коллекция.
//	ПоляВложенныхКоллекций	 - Строка - Имена полей колонок, в которых располагаются вложенные таблицы значений.
//							 - Структура - Элементы:
//									* Ключ - Имя колонки, содержащей таблицу значений. 
//									* Значение - Строка, Структура - см. параметр ПоляВложенныхКоллекций.
//
// Возвращаемое значение:
//	ТаблицаЗначений - Выгруженная таблица.
//
Функция ВыгрузитьКоллекциюФормыРекурсивно(ДанныеФормыКоллекция, Знач ПоляВложенныхКоллекций = "")  
	
	Если ТипЗнч(ПоляВложенныхКоллекций) = Тип("Строка") Тогда
		ПоляВложенныхКоллекций = Новый Структура(ПоляВложенныхКоллекций);
		Для каждого Элемент Из ПоляВложенныхКоллекций Цикл
			ПоляВложенныхКоллекций[Элемент.Ключ] = "";
		КонецЦикла; 
	ИначеЕсли ТипЗнч(ПоляВложенныхКоллекций) = Тип("Структура")	Тогда
		// Ничего
	Иначе
		ВызватьИсключение "Параметр ПоляВложенныхКоллекций: Неожиданный тип значения";
	КонецЕсли;
	
	ТаблицаЗначений = ДанныеФормыКоллекция.Выгрузить(Новый Массив);	// Только колонки    
	
	ПоляОбычныхКолонок = Новый Массив;
	Для каждого Колонка Из ТаблицаЗначений.Колонки Цикл
		ПоляОбычныхКолонок.Добавить(Колонка.Имя);	
	КонецЦикла;
	ПоляОбычныхКолонокСтрокой = СтрСоединить(ПоляОбычныхКолонок, ", ");
	
	Для каждого Поле Из ПоляВложенныхКоллекций Цикл
		ТаблицаЗначений.Колонки.Добавить(Поле.Ключ);
	КонецЦикла;                                     
	
	Для каждого ЭлементКоллекции Из ДанныеФормыКоллекция Цикл
		СтрокаТаблицы = ТаблицаЗначений.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ЭлементКоллекции, ПоляОбычныхКолонокСтрокой);
		
		Для каждого Поле Из ПоляВложенныхКоллекций Цикл
			СтрокаТаблицы[Поле.Ключ] = ВыгрузитьКоллекциюФормыРекурсивно(ЭлементКоллекции[Поле.Ключ], Поле.Значение);
		КонецЦикла;
		
	КонецЦикла;	 
	
	Возврат ТаблицаЗначений;
	
КонецФункции    

// #СДЕЛАТЬ Добавить загрузку других коллекций
// Загружает таблицу значений, содержащую вложенные таблицы в коллекцию данных формы
//
// Параметры:
//	ДанныеФормыКоллекция - ДанныеФормыКоллекция	 - Данные формы.
//	ЗагружаемаяТаблица	 - ТаблицаЗначений		 - Загружаемая таблица.
//
Процедура ЗагрузитьКоллекциюФормыРекурсивно(ДанныеФормыКоллекция, ЗагружаемаяТаблица)

	ДанныеФормыКоллекция.Очистить();
	
	ИменаКолонокТаблицы = Новый Массив;       
	СтруктураПроверки = Новый Структура;
	СтруктураРеференс = Новый Структура;
	Для каждого Колонка Из ЗагружаемаяТаблица.Колонки Цикл
		ИменаКолонокТаблицы.Добавить(Колонка.Имя);		
		СтруктураРеференс.Вставить(Колонка.Имя, новый УникальныйИдентификатор);
		СтруктураПроверки.Вставить(Колонка.Имя, СтруктураРеференс[Колонка.Имя]);
	КонецЦикла;    
	
	ЗаполнитьЗначенияСвойств(СтруктураПроверки, ДанныеФормыКоллекция.Добавить());
	ИменаКолонокКоллекции = Новый Массив;
	Для каждого ИмяКолонки Из ИменаКолонокТаблицы Цикл
		Если СтруктураПроверки[ИмяКолонки] <> СтруктураРеференс[ИмяКолонки] Тогда
			ИменаКолонокКоллекции.Добавить(ИмяКолонки);
		КонецЕсли;
	КонецЦикла;                                                                
	ДанныеФормыКоллекция.Очистить();	
	
	Для каждого СтрокаТаблицы Из ЗагружаемаяТаблица Цикл
		ЭлементКоллекции = ДанныеФормыКоллекция.Добавить();
		Для каждого ИмяКолонки Из ИменаКолонокКоллекции Цикл
			Значение = СтрокаТаблицы[ИмяКолонки];
			Если ТипЗнч(Значение) = Тип("ТаблицаЗначений") Тогда
				ЗагрузитьКоллекциюФормыРекурсивно(ЭлементКоллекции[ИмяКолонки], СтрокаТаблицы[ИмяКолонки]);
			Иначе
				ЭлементКоллекции[ИмяКолонки] = СтрокаТаблицы[ИмяКолонки];
			КонецЕсли;		
		КонецЦикла; 
	КонецЦикла; 

КонецПроцедуры // ЗагрузитьКоллекциюФормыРекурсивно()

// Получает исполняемый запрос динамического списка.
//
// Параметры:
//  ТаблицаФормы - ТаблицаФормы	 - Таблица формы, связанная с динамическим списком.
// 
// Возвращаемое значение:
//  Запрос
//
Функция ПолучитьИсполняемыйЗапросДинамическогоСписка(ТаблицаФормы) Экспорт  
	
	Если ТипЗнч(ТаблицаФормы) <> Тип("ТаблицаФормы") Тогда
		 ВызватьИсключение "Параметр ТаблицаФормы: Неожиданный тип значения";		 
	КонецЕсли;
	
	Попытка
		ИсполняемаяСхема = ТаблицаФормы.ПолучитьИсполняемуюСхемуКомпоновкиДанных();
		ИсполняемыеНастройки = ТаблицаФормы.ПолучитьИсполняемыеНастройкиКомпоновкиДанных();
	Исключение
		ВызватьИсключение "Параметр ТаблицаФормы: Таблица не привязана к динамическому списку";
	КонецПопытки;                                                                              
	
	Если Ложь Тогда 
		ИсполняемаяСхема = Новый СхемаКомпоновкиДанных;
		ИсполняемыеНастройки = Новый НастройкиКомпоновкиДанных;
	КонецЕсли;                                                                                 
	
	КомпоновщикМакета = Новый КомпоновщикМакетаКомпоновкиДанных;
	МакетКомпоновки = КомпоновщикМакета.Выполнить(ИсполняемаяСхема, ИсполняемыеНастройки);
	
	НаборДанныхЗапрос = МакетКомпоновки.НаборыДанных[0];
	Запрос = Новый Запрос;
	Запрос.Текст = НаборДанныхЗапрос.Запрос;
	ОписаниеПараметров = Запрос.НайтиПараметры();
	Для каждого ОписаниеПараметра Из ОписаниеПараметров Цикл
		ИмяПараметра = ОписаниеПараметра.Имя;
		ЗначениеПараметра = МакетКомпоновки.ЗначенияПараметров.Найти(ИмяПараметра);	
		Запрос.УстановитьПараметр(ИмяПараметра, ЗначениеПараметра.Значение);
		Если ЗначениеПараметра = Неопределено Тогда
			ВызватьИсключение СтрШаблон("Не установлен параметр данных «%1»", ИмяПараметра);			
		КонецЕсли;
	КонецЦикла;
	
	Возврат Запрос;
	
КонецФункции

// Создает массив реквизитов формы на основе структуры Таблицы значений.
// Предназначен для добавленя реквизитов на форму.
//
// Параметры:
//  ТаблицаЗначений	 - ТаблицаЗначений	 - Исходная таблица значений, по колонкам которой создаются реквизиты.
//  Имя				 - Строка - Имя базового реквизита для таблицы значений.
//  Путь			 - Строка - Путь к объекту формы (необязательный, по умолчанию пустая строка).
//  Заголовок		 - Строка - Заголовок для базового реквизита (необязательный, по умолчанию пустая строка).
//  СохраняемыеДанные - Булево - Признак сохранения данных реквизита (необязательный, по умолчанию Ложь).
//
// Возвращаемое значение:
//  Массив - массив объектов РеквизитФормы, содержащий реквизит для всей таблицы и для каждой её колонки.
//
Функция РеквизитыФормыИзТаблицыЗначений(ТаблицаЗначений, Имя, Путь = "", Заголовок = "", СохраняемыеДанные = Ложь) Экспорт
	
	РеквизитыФормы = Новый Массив;        
	Реквизит = Новый РеквизитФормы(Имя, Новый ОписаниеТипов("ТаблицаЗначений"), Путь, Заголовок, СохраняемыеДанные);
	РеквизитыФормы.Добавить(Реквизит);
	ПутьКолонок = ?(ЗначениеЗаполнено(Путь), Путь + ".", "") + Имя;
	Для каждого КолонкаТаблицыЗначений Из ТаблицаЗначений.Колонки Цикл
		Реквизит = Новый РеквизитФормы(
		КолонкаТаблицыЗначений.Имя, 
		КолонкаТаблицыЗначений.ТипЗначения, 
		ПутьКолонок, 
		КолонкаТаблицыЗначений.Заголовок, 
		СохраняемыеДанные
		);
		РеквизитыФормы.Добавить(Реквизит);
	КонецЦикла;  
	
	Возврат РеквизитыФормы;

КонецФункции

