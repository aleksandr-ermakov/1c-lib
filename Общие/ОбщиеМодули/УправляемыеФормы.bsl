...
//
Процедура СоздатьЛегендуУсловногоОформления(ГруппаЭлементовЛегенды, ОбластьОформления = Неопределено, Форма = Неопределено)
	...
	Для каждого ЭлементУсловногоОформления Из Форма.УсловноеОформление.Элементы Цикл
		...
		Если ФильтроватьПоЭлементамФормы Тогда
			...
			Для каждого ОформляемоеПоле Из ЭлементУсловногоОформления.Поля.Элементы Цикл
				Если ОформляемоеПоле.Использование И ИменаОформляемыхЭлементовФормы.Найти(Строка(ОформляемоеПоле.Поле)) <> Неопределено Тогда
		...
		Если ЭлементФормы <> Неопределено Тогда
			...
			Для каждого ЭлементРасширеннойПодсказки Из ЭлементыРасширеннойПодсказки Цикл
				Если ЗначениеЗаполнено(ЭлементРасширеннойПодсказки.Значение) Тогда 
					Если ЗначениеЗаполнено(СоставРасширеннойПодсказки) Тогда СоставРасширеннойПодсказки.Добавить(Символы.ПС + Символы.ПС) КонецЕсли; 
					СоставРасширеннойПодсказки.Добавить(Новый ФорматированнаяСтрока(ЭлементРасширеннойПодсказки.Представление + ":	", Новый Шрифт(, , Истина)));
