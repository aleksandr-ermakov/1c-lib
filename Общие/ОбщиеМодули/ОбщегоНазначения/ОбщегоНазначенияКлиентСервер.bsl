...
// Объединяет несколько описаний типов в одно, расширяя доступные квалификаторы
//
// Параметры:
//	ОписаниеТиповА	 - ОписаниеТипов - Описание типов. 
//				 	 - Массив из ОписаниеТипов - Несколько описаний типов для объединения.
...
//
// Варианты вызова:
//	ОбъединитьОписанияТипов(ОписаниеТипов, ОписаниеТипов, ...) - Объединение указанных отдельно описаний типов.
//	ОбъединитьОписанияТипов(Массив) - Объединение типов, переданных в массиве.
//
Функция ОбъединитьОписанияТипов(ОписаниеТиповА, ОписаниеТиповБ = Неопределено) Экспорт

	Если ТипЗнч(ОписаниеТиповА) = Тип("Массив") Тогда
		МассивОписаний = ОписаниеТиповА;
	ИначеЕсли ТипЗнч(ОписаниеТиповА) = Тип("ОписаниеТипов") Тогда
		Если ТипЗнч(ОписаниеТиповБ) <> Тип("ОписаниеТипов") Тогда
			ВызватьИсключение "Параметр ОписаниеТиповБ: Ожидается тип ОписаниеТипов.";
		КонецЕсли;
		МассивОписаний = Новый Массив(2);
		МассивОписаний[0] = ОписаниеТиповА;
		МассивОписаний[1] = ОписаниеТиповБ;
	Иначе
		ВызватьИсключение "Неверные параметры вызова.";
	КонецЕсли;
	...
	Для каждого ТекущееОписаниеТипов Из МассивОписаний Цикл
		...
		Если ТекущееОписаниеТипов.СодержитТип(Тип("Число")) Тогда
			...
			Иначе
				РазрядностьДробнойЧасти = Макс(КвалификаторыЧисла.РазрядностьДробнойЧасти, 
...
КонецФункции // ОбъединитьОписанияТипов()
...
Функция ОписаниеТиповПоСтроке(Знач ОписаниеТиповСтрокой, Знач Разделитель = ";") Экспорт
	...
	ОписаниеТипов = ОбъединитьОписанияТипов(ВсеОписанияТипов);
...
Функция ОписаниеХранимыхТиповСтрокой(ОписаниеТипов) Экспорт
...
Функция ТипВсеСсылки() Экспорт
	...
	НаборыТипов.Добавить(Метаданные.Справочники				.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.Документы				.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.Перечисления			.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.ПланыВидовХарактеристик	.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.ПланыСчетов				.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.ПланыВидовРасчета		.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.ПланыОбмена				.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.БизнесПроцессы			.ТипВсеСсылки());
	НаборыТипов.Добавить(Метаданные.БизнесПроцессы			.ТипВсеСсылкиТочекМаршрутаБизнесПроцессов());
	НаборыТипов.Добавить(Метаданные.Задачи					.ТипВсеСсылки());

	Для каждого НаборТипов Из НаборыТипов Цикл
		Для каждого ТекущийТип Из НаборТипов Цикл
			ВсеТипы.Добавить(ТекущийТип);
		КонецЦикла;
	КонецЦикла;
...
//
Функция ИмяОбъектаИзСинонима(Синоним) Экспорт
