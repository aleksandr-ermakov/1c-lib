...
Функция РимскоеЧисло(Число, Знач Форма = Истина) Экспорт
	...
	Для каждого РазрядСочетаний Из РазрядыСочетаний Цикл
		...
		Для ИндексСочетания = 1 По МаксИндексСочетания Цикл
			НаборРазрядов = ТекущиеСочетания[ИндексСочетания];
			Для НомерСимвола = 1 По СтрДлина(НаборРазрядов) Цикл
				ТекущийРазряд = Сред(НаборРазрядов, НомерСимвола, 1);
	...
	Пока УменьшаемоеЗначение > 0 Цикл
		Для ИндексВычитаемого = НачИндексВычитаемого По Сочетания.Количество() - 1 Цикл
