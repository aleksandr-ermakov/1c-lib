...
Функция ЦветИзHSV(Тон, Насыщенность, Яркость) Экспорт
	...
	Поправка = (Тон / 60) % 2 - 1;	// ( H / 60 °) mod 2 - 1 
	Компонент = Хрома * (1 - ?(Поправка < 0, - Поправка, Поправка)); // X = C × (1 - | ( H / 60 °) mod 2 - 1 |)
	...
	Красный	 = Окр((КрасныйБаза	 + Дополнение) * 255);
	Зеленый	 = Окр((ЗеленыйБаза	 + Дополнение) * 255);
	Синий	 = Окр((СинийБаза	 + Дополнение) * 255);
