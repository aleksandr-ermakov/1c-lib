...
//
Процедура ДополнитьСписокЗначений(
	ДополняемыйСписок, 
	ДополняющийСписок, 
	Пометка = Неопределено, 
	УдалятьОтсутствующие = Ложь, 
	ОбновлятьПредставления = Истина, 
	ОбновлятьПометки = Ложь) Экспорт
	...
	Для каждого ДополняющийЭлемент Из ДополняющийСписок Цикл
		...
		Если ОбновляемыйЭлемент = Неопределено Тогда
			ДополняемыйСписок.Добавить(
				ДополняющийЭлемент.Значение, 
				ДополняющийЭлемент.Представление, 
				?(Пометка = Неопределено, ДополняющийЭлемент.Пометка, Пометка));
			Продолжить;
		Иначе
