...
//
Функция ДобавитьОператорСхемыЗапросаКопированием(ЗапросСхемы, ДобавляемыйЗапрос, ТипОбъединения = Неопределено, СопоставитьПоляПоИменам = Истина, ДобавитьОтсутствующиеПоля = Ложь) Эспорт
	...
	Для каждого ДобавляемаяКолонка Из ДобавляемыеКолонки Цикл
		...
		КонецЕсли; 
		//Если ТипЗнч(ДобавляемоеПоле) = Тип("ВыражениеСхемыЗапроса") Тогда
		//	НовОператорСхемы.ВыбираемыеПоля.Добавить(Строка(ДобавляемоеПоле), Колонка);
		//	
		//ИначеЕсли ТипЗнч(ДобавляемоеПоле) = Тип("ВложеннаяТаблицаСхемыЗапроса") Тогда
		//	ВызватьИсключение "Вы ступаете на неизведанную территорию";	// #СДЕЛАТЬ 
		//	
		//ИначеЕсли ТипЗнч(ДобавляемоеПоле) = Тип("Строка") Тогда	// Для отсутствующих колонок
		//	НовОператорСхемы.ВыбираемыеПоля.Добавить(ДобавляемоеПоле, Колонка);
		//	
		//Иначе
		//	ВызватьИсключение "Неизвестный тип выбираемого поля";
		//	
		//КонецЕсли; 
...
Функция ДобавитьГруппировкуСхемыЗапросаКопированием(ГруппировкиСхемы, КопируемаяГруппировка) Экспорт
	...
	Возврат ГруппировкиСхемы[ГруппировкиСхемы.Количество() - 1];
	
КонецФункции // ДобавитьГруппировкуСхемыЗапросаКопированием()
