...
Функция КодЦвета(Знач Цвет, Префикс = "#", Кратко = Ложь) Экспорт
	...
	База = 16;
	ЦветПоРазрядам = Новый Массив(6);
	ЦветПоРазрядам[0] = Цел(Цвет.Красный / База);	 ЦветПоРазрядам[1] = Цвет.Красный % База;
	ЦветПоРазрядам[2] = Цел(Цвет.Зеленый / База);	 ЦветПоРазрядам[3] = Цвет.Зеленый % База;
	ЦветПоРазрядам[4] = Цел(Цвет.Синий / База);		 ЦветПоРазрядам[5] = Цвет.Синий % База;

	Если Кратко 
		И ЦветПоРазрядам[0] = ЦветПоРазрядам[1]
		И ЦветПоРазрядам[2] = ЦветПоРазрядам[3]
		И ЦветПоРазрядам[4] = ЦветПоРазрядам[5] Тогда

		КодЦвета = XMLСтрока(ЦветПоРазрядам[0]) + XMLСтрока(ЦветПоРазрядам[2]) + XMLСтрока(ЦветПоРазрядам[4]);

	Иначе
		
		КодЦвета = "";
		Для Каждого Разряд Из ЦветПоРазрядам Цикл
			КодЦвета = КодЦвета + XMLСтрока(Разряд);
		КонецЦикла;
