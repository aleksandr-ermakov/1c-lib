...
//
// Параметры:
//  СхемаЗапроса - СхемаЗапроса	 - Схема запроса, из которой будет получен первый оператор выбора схемы запроса.
//
// Возвращаемое значение:
//  - ОператорВыбратьСхемыЗапроса - Найденный оператор.
//	- Неопределено - Если оператор не найден.
//
Функция ПервыйОператорВыбратьСхемыЗапроса(СхемаЗапроса) Экспорт

	Для каждого ЗапросСхемы Из СхемаЗапроса.ПакетЗапросов Цикл
		Если ТипЗнч(ЗапросСхемы) = Тип("ЗапросВыбораСхемыЗапроса") Тогда
			Возврат ЗапросСхемы.Операторы[0];
		КонецЕсли;
	КонецЦикла;
