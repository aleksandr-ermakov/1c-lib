...
//
Функция ЗначениеСвойства(Объект, ИмяСвойства, ЗначениеПоУмолчанию = Неопределено) Экспорт
...
// Вычисляет агрегатную функцию по перечю значений.
//
// Параметры:
//	Значения			 - Массив	 - Значения, к которым применяется агрегатная функция.
...
//
Функция Агрегат(Знач Значения, АгрегатнаяФункция = "Сумма") Экспорт
	...
	ИначеЕсли АгрегатнаяФункцияВрег = "МИНИМУМ"  Тогда
		...
		Для Каждого ТекущееЗначение Из Значения Цикл
			РезультатСравнения = Сравнение.Сравнить(ТекущееЗначение, Минимум);
	...
	ИначеЕсли АгрегатнаяФункцияВрег = "МАКСИМУМ"  Тогда
		...
		Сравнение = Новый СравнениеЗначений;
		Для Каждого ТекущееЗначение Из Значения Цикл
			РезультатСравнения = Сравнение.Сравнить(ТекущееЗначение, Максимум);
			Если РезультатСравнения > 0 Тогда
	...
	ИначеЕсли АгрегатнаяФункцияВрег = "ЛЮБОЙ" Тогда
		...
		Для Каждого ТекущееЗначение Из Значения Цикл
			Если ТипЗнч(ТекущееЗначение) = Тип("Булево") И ТекущееЗначение = Истина Тогда
				Возврат Истина;
			КонецЕсли;
		КонецЦикла;
		Возврат Ложь;
	...
	ИначеЕсли АгрегатнаяФункцияВрег = "КАЖДЫЙ" Тогда
		...
		Для Каждого ТекущееЗначение Из Значения Цикл
			Если ТипЗнч(ТекущееЗначение) = Тип("Булево") И ТекущееЗначение = Ложь Тогда
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
		Возврат Истина; 
...
//	АгрегатнаяФункция	 - Строка		 - см. Агрегат()
...
Функция АгрегатКоллекции(Знач Коллекция, АгрегатнаяФункция = "Сумма", Поле = Неопределено) Экспорт
	...
	Агрегат = Агрегат(Значения, АгрегатнаяФункция);
	Возврат Агрегат;

КонецФункции

// Рассчитывает сумму полей элементов коллекции.
//
// Параметры: 
//	Коллекция	 - Массив, Произвольный - Коллекция элементов с полями, или Массив значений.
//	Поле		 - Строка, Число - Поле коллекции. Если не указан, агрегат считается по элементам переданного массива.
//
// Возвращаемое значение:
//	Число - Для пустой коллекции возвращается 0.
//
Функция Сумма(Коллекция, Поле = Неопределено) Экспорт
	Возврат АгрегатКоллекции(Коллекция, "Сумма", Поле);
КонецФункции

// Рассчитывает среднее значение поля элементов коллекции.
//
// Параметры: 
//	Коллекция	 - Массив, Произвольный - Коллекция элементов с полями, или Массив значений.
//	Поле		 - Строка, Число - Поле коллекции. Если не указан, агрегат считается по элементам переданного массива.
...
//	Число - Для пустой коллекции возвращается 0.
//
Функция Среднее(Коллекция, Поле = Неопределено) Экспорт
	Возврат АгрегатКоллекции(Коллекция, "Среднее", Поле);
КонецФункции

// Рассчитывает минимальное значение поля элементов коллекции.
//
// Параметры: 
//	Коллекция	 - Массив, Произвольный - Коллекция элементов с полями, или Массив значений.
//	Поле		 - Строка, Число - Поле коллекции. Если не указан, агрегат считается по элементам переданного массива.
...
//
Функция Минимум(Коллекция, Поле = Неопределено) Экспорт
	Возврат АгрегатКоллекции(Коллекция, "Минимум", Поле);
КонецФункции

// Рассчитывает максимальное значение поля элементов коллекции.
//
// Параметры: 
//	Коллекция	 - Массив, Произвольный - Коллекция элементов с полями, или Массив значений.
//	Поле		 - Строка, Число - Поле коллекции. Если не указан, агрегат считается по элементам переданного массива.
...
//	Число, Строка, Дата, Булево - Для пустой коллекции возвращается Неопределено. Для сервера, см. СравнениеЗначений.
//
Функция Максимум(Коллекция, Поле = Неопределено) Экспорт
	Возврат АгрегатКоллекции(Коллекция, "Максимум", Поле);
КонецФункции
...
Функция Количество(Коллекция) Экспорт
	...
	КонецЦикла;
	Возврат АгрегатКоллекции(ЭлементыКоллекции, "Количество");
КонецФункции

// Рассчитывает количество различных элементов коллекции.
//
// Параметры: 
//	Коллекция	 - Массив, Произвольный - Коллекция элементов с полями, или Массив значений.
//	Поле		 - Строка, Число - Поле коллекции. Если не указан, агрегат считается по элементам переданного массива.
//
// Возвращаемое значение:
//	Число - Количество различных элементов коллекции.
//
Функция КоличествоРазличных(Коллекция, Поле = Неопределено) Экспорт
	Возврат АгрегатКоллекции(Коллекция, "КоличествоРазличных", Поле);
КонецФункции

// Проверяет, содержится ли булево значение Истина в коллекции.
//
// Параметры: 
//	Коллекция	 - Массив, Произвольный - Коллекция элементов с полями, или Массив значений.
//	Поле		 - Строка, Число - Поле коллекции. Если не указан, агрегат считается по элементам переданного массива.
...
//
Функция Любой(Коллекция, Поле = Неопределено) Экспорт
	Возврат АгрегатКоллекции(Коллекция, "Любой", Поле);
КонецФункции

// Проверяет, все ли булевы значения в коллекции равны Истина.
//
// Параметры: 
//	Коллекция	 - Массив, Произвольный - Коллекция элементов с полями, или Массив значений.
//	Поле		 - Строка, Число - Поле коллекции. Если не указан, агрегат считается по элементам переданного массива.
...
//
Функция Каждый(Коллекция, Поле = Неопределено) Экспорт
	Возврат АгрегатКоллекции(Коллекция, "Каждый", Поле);
КонецФункции
...
// Сохраняет порядок следования значений в исходном массиве до удаления.
//
// Параметры:
//  ИсходныйМассив 	 - Массив - Обрабатываемый массив
//  Значения		 - Массив - Значения к удалению.
//
Процедура УдалитьЗначенияМассива(ИсходныйМассив, Значения) Экспорт
...
КонецПроцедуры // УдалитьЗначенияМассива()
