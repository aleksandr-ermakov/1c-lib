...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ОбъединитьОписанияТипов");
	...
	#Область Конвертация
	ВсеТесты.Добавить("ИмяИзСинонима");
	ВсеТесты.Добавить("СинонимИзИмени");
...
Процедура ОбъединитьОписанияТипов() Экспорт
	...
	ОписаниеТиповРезультат = Модуль.ОбъединитьОписанияТипов(ОписаниеТиповА, ОписаниеТиповБ);
...
КонецПроцедуры // ОбъединитьОписанияТипов()
...
Процедура ИмяИзСинонима() Экспорт
	...
	Для каждого Пример Из Примеры Цикл
		Результат = Модуль.ИмяИзСинонима(Пример.Ключ);
...
КонецПроцедуры // ИмяИзСинонима()

Процедура СинонимИзИмени() Экспорт
	Примеры = Новый Соответствие;
	Примеры[""] = "";
	Примеры["Слово"] = "Слово";
	Примеры["слово"] = "Слово";
	...
	Примеры["_АаАаААааАААаааААААааааААААА_АА_ААА_123_"] = "Аа аа а ааа АААааа ААААаааа ААААА а а ААА 123";

	Для каждого Пример Из Примеры Цикл
		Результат = Модуль.СинонимИзИмени(Пример.Ключ);
		Эталон = Пример.Значение;
		ТекстСообщения = СтрШаблон("""%1"" -> ""%2""", Пример.Ключ, Пример.Значение);
		Ожидаем.Что(Результат, ТекстСообщения).Равно(Эталон);
	КонецЦикла;
