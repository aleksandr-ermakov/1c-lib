...
Перем ТипГруппировкиКомпоновкиДанных;

#КонецОбласти // OneScript

// Устарела. см. УстановитьТипКолонкиТаблицыЗначений()
//
...
//								 - КолонкаТаблицыЗначений		 - Изменяемая колонка
...
// Проверяет уникальность значений в указанных колонках таблицы значений.
//
// Параметры:
//  ТаблицаЗначений		 - ТаблицаЗначений		 - Проверяемая таблица.
//  Колонки				 - Строка				 - Имена колонок, разделенные запятой.
//	НеуникальныеЗначения - Массив из Структура	 - Неуникальные значения. Элемент: Структура, ключи - имена колонок.
//
// Возвращаемое значение:
//   Булево   - Истина, если значения уникальны.
//
Функция ПроверитьУникальностьЗначенийТаблицы(ТаблицаЗначений, Колонки, НеуникальныеЗначения = Неопределено) Экспорт

	Если ПустаяСтрока(Колонки) Тогда
		ВызватьИсключение "Параметр Колонки: Ожидается непустая строка с именами колонок, разделенных запятой.";
	КонецЕсли;
	
	Попытка
		КолонкиСтруктурой = Новый Структура();
	Исключение
	    ВызватьИсключение "Параметр Колонки: Неожиданное значение";
	КонецПопытки;
	ИмяКолонкиКоличество = "Количество";
	Пока КолонкиСтруктурой.Свойство(ИмяКолонкиКоличество) Цикл
		ИмяКолонкиКоличество = ИмяКолонкиКоличество + СтрЗаменить(Новый УникальныйИдентификатор, "-", "");	
	КонецЦикла;  
	
	ПроверочнаяТаблица = ТаблицаЗначений.Скопировать(, Колонки);
	ПроверочнаяТаблица.Колонки.Добавить(ИмяКолонкиКоличество, Новый ОписаниеТипов("Число"));
	ПроверочнаяТаблица.ЗаполнитьЗначения(1, ИмяКолонкиКоличество);
	ПроверочнаяТаблица.Свернуть(Колонки, ИмяКолонкиКоличество);

	НеуникальныеЗначения = Новый Массив;
	Для каждого СтрокаТаблицы Из ПроверочнаяТаблица Цикл
		Если СтрокаТаблицы[ИмяКолонкиКоличество] > 1 Тогда
			НеуникальноеЗначение = Новый Структура(Колонки);
			ЗаполнитьЗначенияСвойств(НеуникальноеЗначение, СтрокаТаблицы);
			НеуникальныеЗначения.Добавить(НеуникальноеЗначение);
		КонецЕсли;
	КонецЦикла;

	Возврат Не ЗначениеЗаполнено(НеуникальныеЗначения); 
...
Функция СвернутьТаблицуЗначений(Знач ТаблицаЗначений, Измерения = "", Ресурсы = "") Экспорт
	...
	Если ТипЗнч(Измерения) = Тип("Строка") Тогда
		...
		Для каждого ИмяИзмерения Из ИменаИзмерений Цикл
			...
			ПозПсевдоним = СтрНайти(ВРег(Выражение), ВРег(РазделительКак));
			ЕстьПсевдоним = Булево(ПозПсевдоним);    
			
			Если ЕстьПсевдоним Тогда
				ИмяПоля = Сред(Выражение, ПозПсевдоним + СтрДлина(РазделительКак));
			Иначе	
	...
	Иначе
		...
		ВызватьИсключение "Параметр Измерения: Ожидается Строка, Структура, Соответствие";
	...
	Иначе
		ВызватьИсключение "Параметр Ресурсы: Ожидается Строка, Структура, Соответствие"; 
	...
	ИмяНабораДанных			 = "ТаблицаЗначений";
	ВнешниеНаборыДанных		 = Новый Структура;
	ВнешниеНаборыДанных.Вставить(ИмяНабораДанных, ТаблицаЗначений);
	...
	НаборДанных.ИмяОбъекта		= НаборДанных.Имя;
...
// Формирует иерархическое дерево по указанным полям таблицы значений.
//
// Параметры:
//  ИсходнаяТаблица	 - ТаблицаЗначений	 - Исходная таблица
//  ПоляИерархии	 - Структура		 - Структура с одним элементом:
...
// 
// Возвращаемое значение:
//   - ДеревоЗначений   - Иерархия значений таблицы
//
Функция ПостроитьИерархиюТаблицыЗначений(ИсходнаяТаблица, ПоляИерархии, Ресурсы = "")
	
	Если ТипЗнч(ПоляИерархии) <> Тип("Структура")
		Или ПоляИерархии.Количество() <> 1 Тогда
		ВызватьИсключение "Параметр ПоляИерархии: Ожидается Структура с одним элементом.";
	КонецЕсли;	

	Для каждого ЭлементСтруктуры Из ПоляИерархии Цикл
	    ИмяПоляПотомка	 = ЭлементСтруктуры.Ключ;
		ИмяПоляРодителя	 = ЭлементСтруктуры.Значение;
		Прервать;
	КонецЦикла; 
	
	#Область ТаблицаИерархии
	ИменаКолонок = ИмяПоляПотомка + ", " + ИмяПоляРодителя;
	ТаблицаИерархии = ИсходнаяТаблица.СкопироватьКолонки(ИменаКолонок);
	РодителиЭлементов = Новый Соответствие;  
	Для каждого СтрокаТаблицы Из ИсходнаяТаблица Цикл
		РодителиЭлементов[СтрокаТаблицы[ИмяПоляПотомка]] = СтрокаТаблицы[ИмяПоляРодителя];
	КонецЦикла;    
	ВсеРодители = Новый Соответствие;	// Родители и пустые значения их типов   
	ПустыеЗначенияТипов = Новый Соответствие;
	Для каждого РодительЭлемента Из РодителиЭлементов Цикл
		Родитель = РодительЭлемента.Значение;    
		Тип = ТипЗнч(Родитель);
		ПустоеЗначениеТипа = ПустыеЗначенияТипов[Тип];
		Если ПустоеЗначениеТипа = Неопределено И Родитель <> Неопределено Тогда
			МассивТипов = Новый Массив;
			МассивТипов.Добавить(Тип);
			ОписаниеТипов = Новый ОписаниеТипов(МассивТипов);
			ПустоеЗначениеТипа = ОписаниеТипов.ПривестиЗначение();
			ПустыеЗначенияТипов[Тип] = ПустыеЗначенияТипов;
		КонецЕсли;                                                 
		ВсеРодители[Родитель] = ПустоеЗначениеТипа;
	КонецЦикла; 
	Для каждого ЭлементСоответствия Из ВсеРодители Цикл
		Родитель = ЭлементСоответствия.Ключ;
		Если РодителиЭлементов[Родитель] = Неопределено Тогда
			РодителиЭлементов[Родитель] = ЭлементСоответствия.Значение;
		КонецЕсли;		
	КонецЦикла;
	Для каждого РодительЭлемента Из РодителиЭлементов Цикл
		СтрокаТаблицы = ТаблицаИерархии.Добавить();
		СтрокаТаблицы[ИмяПоляПотомка]	 = РодительЭлемента.Ключ;
		СтрокаТаблицы[ИмяПоляРодителя]	 = РодительЭлемента.Значение;
	КонецЦикла; 
	#КонецОбласти // ТаблицаИерархии
	...
	СхемаКомпоновкиДанных	 = Новый СхемаКомпоновкиДанных;
	ИмяНабораДанныхТаблица	 = "ТаблицаЗначений";
	ИмяНабораДанныхИерархия	 = "Иерархия";
	ВнешниеНаборыДанных		 = Новый Структура;
	ВнешниеНаборыДанных.Вставить(ИмяНабораДанныхТаблица, ИсходнаяТаблица);
	ВнешниеНаборыДанных.Вставить(ИмяНабораДанныхИерархия, ТаблицаИерархии);
	ВозможностьИспользованияВнешнихФункций = Истина;
	...
	Для каждого ОписаниеВнешнегоНабораДанных Из ВнешниеНаборыДанных Цикл
		НаборДанных = СхемаКомпоновкиДанных.НаборыДанных.Добавить(Тип("НаборДанныхОбъектСхемыКомпоновкиДанных"));
		НаборДанных.Имя				= ОписаниеВнешнегоНабораДанных.Ключ;
		НаборДанных.ИсточникДанных	= ИсточникДанныхСхемы.Имя;
		НаборДанных.ИмяОбъекта		= НаборДанных.Имя;
		Для каждого Колонка Из ОписаниеВнешнегоНабораДанных.Значение.Колонки Цикл
			ПолеДанных				= НаборДанных.Поля.Добавить(Тип("ПолеНабораДанныхСхемыКомпоновкиДанных"));
			ПолеДанных.Поле			= Колонка.Имя;
			ПолеДанных.ПутьКДанным	= Колонка.Имя;
			ПолеДанных.ТипЗначения	= Колонка.ТипЗначения;
	...
	СвязьНаборовДанных.НаборДанныхИсточник	 = ИмяНабораДанныхТаблица;
	СвязьНаборовДанных.НаборДанныхПриемник	 = ИмяНабораДанныхИерархия;
	СвязьНаборовДанных.ВыражениеИсточник	 = ИмяПоляПотомка;
	СвязьНаборовДанных.ВыражениеПриемник	 = ИмяПоляПотомка; 
	СвязьНаборовДанных.Обязательная			 = Истина;
	
	// Иерархия, к себе:
	СвязьНаборовДанных = СхемаКомпоновкиДанных.СвязиНаборовДанных.Добавить();
	СвязьНаборовДанных.НаборДанныхИсточник	 = ИмяНабораДанныхИерархия;
	СвязьНаборовДанных.НаборДанныхПриемник	 = ИмяНабораДанныхИерархия;
	СвязьНаборовДанных.ВыражениеИсточник	 = ИмяПоляРодителя;
	СвязьНаборовДанных.ВыражениеПриемник	 = ИмяПоляПотомка;
	СвязьНаборовДанных.Обязательная			 = Истина;
	#КонецОбласти // СвязиНаборовДанных
	
	#Область ВычисляемыеПоля
	// Для ресурсов с собственными псевдонимами добавляем вычисляемые поля
	ВычисляемыеПоля = СхемаКомпоновкиДанных.ВычисляемыеПоля;    
	ПоляНабораДанных = НаборДанных.Поля;
	...
	#Область Структура
	СтруктураВарианта = СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Структура;
	
	// Иерархия
	Группировка = СтруктураВарианта.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
	ПолеГруппировки = Группировка.ПоляГруппировки.Элементы.Добавить(Тип("ПолеГруппировкиКомпоновкиДанных"));
	ПолеГруппировки.Поле = Новый ПолеКомпоновкиДанных(ИмяПоляПотомка);
	ПолеГруппировки.ТипГруппировки = ТипГруппировкиКомпоновкиДанных.Иерархия;  
	Группировка.Выбор.Элементы.Добавить(Тип("АвтоВыбранноеПолеКомпоновкиДанных"));
	ГруппировкаИерархия = Группировка;
	
	// Детальные                                                                  
	Группировка = ГруппировкаИерархия.Структура.Добавить(Тип("ГруппировкаКомпоновкиДанных"));
	Группировка.ПоляГруппировки.Элементы.Добавить(Тип("АвтоПолеГруппировкиКомпоновкиДанных"));
	Группировка.Выбор.Элементы.Добавить(Тип("АвтоВыбранноеПолеКомпоновкиДанных"));    
	
	// Выбранные поля:
	ПоляВыбора = СхемаКомпоновкиДанных.НастройкиПоУмолчанию.Выбор.Элементы;
	НаборДанныхТаблица = СхемаКомпоновкиДанных.НаборыДанных.Найти(ИмяНабораДанныхТаблица);
	Для каждого ПолеНабора Из НаборДанныхТаблица.Поля Цикл
		ПолеВыбора			 = ПоляВыбора.Добавить(Тип("ВыбранноеПолеКомпоновкиДанных"));
		ПолеВыбора.Поле		 = Новый ПолеКомпоновкиДанных(ПолеНабора.Поле);
		ПолеВыбора.Заголовок = ПолеНабора.Заголовок;    
	КонецЦикла;
...
КонецФункции // ПостроитьИерархиюТаблицыЗначений()  
...
// Устанавливает Тип колонки таблицы значений.
//  Порядок колонок и индексы таблицы сохраняются.
//
// Параметры:
//  ТаблицаЗначений				 - ТаблицаЗначений				 - Изменяемая таблица
...
//								 - КолонкаТаблицыЗначений		 - Изменяемая колонка
//								 - Число						 - Индекс изменяемой колонки
//  ТипЗначения					 - ОписаниеТипов				 - Устанавливаемый тип значеиня
//
Процедура УстановитьТипКолонкиТаблицыЗначений(ТаблицаЗначений, Колонка, ТипЗначения) Экспорт
	
	Если ТипЗнч(ТаблицаЗначений) <> Тип("ТаблицаЗначений") Тогда
		ВызватьИсключение "Параметр ТаблицаЗначений: Ожидается тип ТаблицаЗначений.";
	КонецЕсли;
	...
	#Область ДобавлениеКолонки
	НоваяКолонка = ТаблицаЗначений.Колонки.Добавить(, ТипЗначения, Колонка.Заголовок, Колонка.Ширина);
	МаксИндексКолонок = ТаблицаЗначений.Колонки.Количество() - 1;
	ТаблицаЗначений.ЗагрузитьКолонку(ТаблицаЗначений.ВыгрузитьКолонку(Колонка), НоваяКолонка);
	ТаблицаЗначений.Колонки.Сдвинуть(НоваяКолонка, 	ИндексКолонки - МаксИндексКолонок + 1);
	ИмяКолонки = Колонка.Имя;
...
КонецПроцедуры // УстановитьТипКолонкиТаблицыЗначений
