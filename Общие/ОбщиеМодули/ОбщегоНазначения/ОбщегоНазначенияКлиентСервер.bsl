...
Процедура ЗаполнитьПометкиДереваВверх(Узел, ИмяПоля = "Пометка", Знач ВключаяТекущий = Истина) Экспорт
	...
	Ветви = ВетвиДерева(Узел); 
	Если ЗначениеЗаполнено(Ветви) И ВключаяТекущий Тогда
		Для каждого Ветвь Из Ветви Цикл  
			ЕстьПомеченные	 = ЕстьПомеченные Или Ветвь[ИмяПоля];
			ЕстьНеПомеченные = ЕстьНеПомеченные Или Не Ветвь[ИмяПоля];
	...
	Если Родитель <> Неопределено Тогда
		ЗаполнитьПометкиДереваВверх(Родитель, ИмяПоля, Истина);
	КонецЕсли;
...
Процедура УстановитьПометкиДереваВниз(Узел, Пометка, ИмяПоля = "Пометка", Знач ВключаяТекущий = Истина) Экспорт
	...
	Если ВключаяТекущий Тогда
		Узел[ИмяПоля] = Булево(Пометка);
	КонецЕсли;

	Ветви = ВетвиДерева(Узел);
	Для каждого Ветвь Из Ветви Цикл
		УстановитьПометкиДереваВниз(Ветвь, Пометка, ИмяПоля, Истина);
	КонецЦикла;
...
//
Функция МассивыРавны(ПервыйМассив, ВторойМассив, ТекстОшибки = "") Экспорт
...
//  ИмяПоля		  - Строка		 - Выгружаемое поле (колонка). Должно присутствовать во всех элементах.
//
// Возвращаемое значение:
//   Массив   - Выгруженные значения поля в порядке следования в коллекции.
//
Функция ВыгрузитьПолеКоллекции(Коллекция, ИмяПоля) Экспорт
...
// Меняет местами ключи и значения элементов Структуры ил Соответствия.
...
Функция СкопироватьКоллекцию(Коллекция, Рекурсивно = Ложь) Экспорт
	...
	Если ТипКоллекции = Тип("ДеревоЗначений") Тогда
		...
		КонецЦикла;
		// СоответствиеСтрок = Новый Соответствие;	// {Строка коллекции; Строка новой коллекции}
		...
		Для каждого ТекущаяСтрока Из Коллекция.Строки Цикл
			...
			// РодителиСтрок[ТекущаяСтрока] = Коллекция;
		КонецЦикла;
		...
		Пока Индекс <= Стек.ВГраница() Цикл
			...
			// ТекущийРодитель = РодителиСтрок[ТекущаяСтрока];
			// НовыйРодитель = СоответствиеСтрок[ТекущийРодитель];
			...
			КонецЦикла;
			// СоответствиеСтрок[ТекущаяСтрока] = НоваяСтрока;
			// РодителиСтрок[НоваяСтрока] = НовыйРодитель;
			Для каждого ПодчиненнаяСтрока Из ТекущаяСтрока.Строки Цикл
				...
				// РодителиСтрок[ПодчиненнаяСтрока] = ТекущаяСтрока;
			КонецЦикла;
			Индекс = Индекс + 1;
		КонецЦикла;
		Возврат НоваяКоллекция;
