...
Функция ОписаниеХранимыхТиповСтрокой(ОписаниеТипов, Разделитель = "; ", Кратко = Ложь) Экспорт
	...
	Для каждого ТекущийТип Из Коллекция Цикл
	...
	Для каждого ТекущийТип Из ОписаниеХранимыхТипов.Типы() Цикл
		...
		Если ИмяТипа <> Неопределено Тогда
			...
			ТипСтрокой = ОписаниеТипаСтрокой(ОписаниеТипа);
		Иначе
			ТипСтрокой = ОписаниеТипаСтрокой(ТекущийТип);
		КонецЕсли;
...
//	Кратко	- Булево		 - Если Истина, то вместо краткого представления типа формируется полное:
//								"ДопустимаяДлина.Фиксированная" → "Фиксированная"
//
// Возвращаемое значение:
//	Строка - Представление типа.
//
Функция ОписаниеХранимогоТипаСтрокой(Тип, Кратко = Ложь) Экспорт
