...
//
// Параметры:
//	ДатаВремя	 - Дата		 - Дата со временем для округления.
//	ШагСекунд	 - Число	 - Шаг округления в секундах. 
//	Режим		 - РежимОкругления, Число - Определяет режим округления. См. Окр().
//	ТочкаОтсчета - Дата		 - Точка отсчёта для округления. Округление происходит относительно этой даты.
//							Можно указать смещение от начала суток.
//							Если не указана - Пустая дата (1.01.0001 00:00).
//
// Возвращаемое значение:
//	Дата - Округлённая дата.
//
Функция ОкруглитьВремя(ДатаВремя, ШагСекунд, Режим = 1, ТочкаОтсчета = '00010101') Экспорт
	
	Если ШагСекунд <= 0 Тогда
		ВызватьИсключение "Шаг должен быть положительным числом.";
	КонецЕсли;

	СекундВСутках = 24 * 60 * 60;
	Если ШагСекунд > СекундВСутках Тогда
		ВызватьИсключение "Шаг не должен превышать количество секунд в сутках.";
	КонецЕсли;

	Если ТипЗнч(ТочкаОтсчета) <> Тип("Дата") Тогда
		ВызватьИсключение "Параметр ТочкаОтсчета: Ожидается дата.";
	КонецЕсли;
	
	Секунд = ДатаВремя - ТочкаОтсчета;

	Результат = ДатаВремя
	- Секунд % ШагСекунд	// Округление вниз.
	+ ШагСекунд * Окр((Секунд % ШагСекунд) / ШагСекунд, 0, Режим);	// Округление вверх/ближайшее.
...
//	Дата - Округлённая дата.
//
Функция ОкруглитьВремяВверх(ДатаВремя, ШагСекунд, ТочкаОтсчета = '00010101') Экспорт
	...
	Результат = ДатаВремя
	- Секунд % ШагСекунд	// Округление вниз.
	+ ?(Секунд % ШагСекунд = 0, 0, ШагСекунд); // Округление вверх.
...
// Параметры:
//   Число        - Число 					 - Исходное число.
//   Шаг          - Число					 - Шаг округления (например, 0.25). Должен быть положительным числом.
//   Режим        - РежимОкругления, Число 	 - См. Окр().
//   Смещение     - Число 					 - Смещение относительно нуля (по умолчанию 0).
//											Модуль смещения не должен превышать модуль числа.
//
// Возвращаемое значение:
//   Число - Округлённое число.
//
Функция ОкруглитьЧислоСШагом(Знач Число, Шаг = 1, Режим = 1, Смещение = 0) Экспорт
    
    Если Шаг <= 0 Тогда
        ВызватьИсключение "Шаг должен быть положительным числом.";
    КонецЕсли;

	Если ?(Смещение < 0, - Смещение, Смещение) > ?(Число < 0, - Число, Число)  Тогда
		ВызватьИсключение "Модуль смещения не должен превышать модуль числа.";
	КонецЕсли;

	Знак = ?(Число < 0, -1, 1);

    Результат = Число - Смещение; // Смещаем относительно нуля
	...
	Результат = Результат + Смещение; // Возвращаем смещение
	Результат = Результат * Знак; // Возвращаем знак

    Возврат Результат;

КонецФункции // ОкруглитьЧислоСШагом()
...
//   Число - Округлённое число.
//
Функция ОкруглитьЧислоВверхСШагом(Знач Число, Шаг = 1, Смещение = 0) Экспорт
