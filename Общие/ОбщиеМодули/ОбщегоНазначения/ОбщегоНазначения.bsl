...
//	ПараметрыЗаписи		 - Структура		 - Используется для перезаписи документов. Это параметры метода Записать():
...
//
Процедура ЗаписатьЗначенияРеквизитовОбъекта(
	Ссылка, 
	ЗначенияРеквизитов, 
	...
	Если ЭтоДокументСсылка Тогда
		...
		Если ТипЗнч(ПараметрыЗаписи) = Тип("Структура") Тогда
			ЗаполнитьЗначенияСвойств(ПараметрыЗаписиДокумента, ПараметрыЗаписи)
		КонецЕсли;
		Если ТипЗнч(ПараметрыЗаписи) = Тип("Булево") И ПараметрыЗаписи = Истина Тогда
			Если Объект.Проведен Тогда
				ПараметрыЗаписиДокумента["РежимЗаписи"] = РежимЗаписиДокумента.Проведение;
			КонецЕсли;
			Если Объект.Дата >= ТекущаяДатаСеанса() Тогда
				...
				Если Ссылка.Метаданные().ОперативноеПроведение = ВариантыОперативногоПроведения.Разрешить Тогда
					ПараметрыЗаписиДокумента["РежимПроведения"] = РежимПроведенияДокумента.Оперативный;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		Объект.Записать(ПараметрыЗаписиДокумента.РежимЗаписи, ПараметрыЗаписиДокумента.РежимПроведения);
...
КонецПроцедуры // ЗаписатьЗначенияРеквизитовОбъекта()
