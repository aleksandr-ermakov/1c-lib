...
Функция НайтиЛистьяДерева(Узел, ПараметрыОтбора) Экспорт
	...
	Если ЗначениеЗаполнено(Коллекция) Тогда
		Для каждого Ветвь Из Коллекция Цикл
			Для каждого Лист Из ЛистьяДерева(Ветвь, ПараметрыОтбора) Цикл 
				ВозвращаемоеЗначение.Добавить(Лист);
			КонецЦикла;
		КонецЦикла;
	Иначе
...
//  ПараметрыОтбора						 - Структура									 - ПараметрыОтбора узлов для получения.
//
// Возвращаемое значение:
//  Массив - Структуры данных
//
Функция ПоляЛистьевДерева(Узел, Поля = "", ПараметрыОтбора = Неопределено) Экспорт

	ЕстьПараметрыОтбора = ПараметрыОтбора <> Неопределено;

	КоллекцияДанных = Новый Массив;
	КоллекцияПодчиненных = ВетвиДерева(Узел);

	Листья = ?(ЕстьПараметрыОтбора, КоллекцияПодчиненных.НайтиСтроки(ПараметрыОтбора, Истина), ЛистьяДерева(Узел));

	Для каждого Лист Из Листья Цикл
		Если ЕстьПараметрыОтбора И Не ЭтоЛистДерева(Лист) Тогда 
			Продолжить; 
		КонецЕсли;	// По ПараметрыОтборау могут быть подобраны узлы, имеющие подчиненные элементы.
...
//  ПараметрыОтбора						 - Структура									 - Какие элементы опрашиваются
//
// Возвращаемое значение:
//  Массив - Структуры данных
//
Функция ПоляДанныхДереваРекурсивно(Узел, Поля = "Идентификатор, ИдентификаторСтроки", ПараметрыОтбора = Неопределено) Экспорт

	КоллекцияДанных = Новый Массив;

	Если ЭтоУзелДерева(Узел) и ЭлементСоответствуетПараметрыОтборау(Узел, ПараметрыОтбора) Тогда
		СтруктураДанных = Новый Структура(Поля);
	...
	Для каждого Ветвь Из Коллекция Цикл
		ДанныеПодчиненных = ПоляДанныхДереваРекурсивно(Ветвь, Поля, ПараметрыОтбора); 
...
Функция НайтиЭлементыКоллекции(
	...
	// Для пустого ПараметрыОтбораа - пустой результат, как для ТаблицаЗначений.НайтиСтроки():
	...
	Для каждого ЭлементКоллекции Из Коллекция Цикл
		...
		Иначе // Пустая структура ПараметрыОтбораа

			Прервать;

		КонецЕсли;

		Если ЭлементСоответствуетПараметрыОтборау Тогда
...
// Проверяет элемент данных на соответствие ПараметрыОтборау.
//
// Параметры:
//  Элемент	 - Произвольный	 - Произвольная коллекция, к которой возможно обращение по имени поля. 
//							 Например, СтрокаДереваЗначений.
//  ПараметрыОтбора	 - Структура	 - Накладываемый ПараметрыОтбора. 
//							 Поля ПараметрыОтбораа должны присутствовать в проверяемом элементе. 
//							 Если Неопределено - элемент считается соответствующим ПараметрыОтборау.
// 
// Возвращаемое значение:
//  Булево - Истина, если элемент соответствует ПараметрыОтборау или ПараметрыОтбора не определён
//
Функция ЭлементСоответствуетПараметрыОтборау(Знач Элемент, Знач ПараметрыОтбора = Неопределено) Экспорт

	Если ПараметрыОтбора = Неопределено Тогда 
		Возврат Истина;
	КонецЕсли; 
	
	Для каждого ЭлементПараметрыОтбораа Из ПараметрыОтбора Цикл
		Если Элемент[ЭлементПараметрыОтбораа.Ключ] <> ЭлементПараметрыОтбораа.Значение Тогда 
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла; 
...
КонецФункции // ЭлементСоответствуетПараметрыОтборау()
