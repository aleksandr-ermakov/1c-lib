...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ВыровнятьДату");
	...
	ВсеТесты.Добавить("ПорцииЭлементовКоллекции");
	...
	ВсеТесты.Добавить("СравнитьНомераВерсий");
...
Процедура ВыровнятьДату() Экспорт

	ИмяМетода = "ВыровнятьДату";
	...
	Режим = 0;	// Вниз.
	Параметры = Новый Массив(1);
	Результат = Модуль.ВыровнятьДату(Дата, ШагСекунд, Режим);
	Ожидаем
		.Что(Результат, "Дата округляется вниз по часу")
		.Равно('20010101110000');
	...
	Результат = Модуль.ВыровнятьДату(Дата, ШагСекунд, Режим);
	Ожидаем
		.Что(Результат, "Дата округляется обычно по часу")
		.Равно('20010101110000');
...
Процедура ПорцииЭлементовКоллекции() Экспорт
	...
	Результат = Модуль.ПорцииЭлементовКоллекции(Массив, 3);
	Ожидаем
		.Что(Результат, "Разбиение на порции размера 3")
		.ИмеетТип("Массив")
		.ИмеетДлину(3);

	Ожидаем.Что(Результат[0]).ИмеетТип("Массив").ИмеетДлину(3);
	Ожидаем.Что(Результат[1]).ИмеетТип("Массив").ИмеетДлину(3);
	Ожидаем.Что(Результат[2]).ИмеетТип("Массив").ИмеетДлину(1);

	Ожидаем.Что(Результат[0][0]).Равно(1);
	Ожидаем.Что(Результат[0][1]).Равно(2);
	Ожидаем.Что(Результат[0][2]).Равно(3);
	Ожидаем.Что(Результат[1][0]).Равно(4);
	Ожидаем.Что(Результат[1][1]).Равно(5);
	Ожидаем.Что(Результат[1][2]).Равно(6);
	Ожидаем.Что(Результат[2][0]).Равно(7);

	Результат = Модуль.ПорцииЭлементовКоллекции(Массив, 10);
	Ожидаем
		.Что(Результат, "Порция больше длины коллекции")
		.ИмеетТип("Массив")
		.ИмеетДлину(1);
	Ожидаем.Что(Результат[0]).ИмеетДлину(7);
...
Процедура СравнитьНомераВерсий() Экспорт
	
	ИмяМетода = "СравнитьНомераВерсий";
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1.0.0", "1.0.0"), "Равные версии")
		.Равно(0);
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1.0.1", "1.0.0"), "Первая версия больше второй")
		.Больше(0);
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1.0.0", "1.0.1"), "Первая версия меньше второй")
		.Меньше(0);
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1.2.0", "1.10.0"), "Первая версия меньше второй по второму числу")
		.Меньше(0);
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("2.0", "1.9.9"), "Первая версия больше второй по первому числу")
		.Больше(0);

	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("3.a", "3.a"), "Версии с буквами равны")
		.Равно(0);
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("3.a", "3.b"), "Версия с 'a' меньше версии с 'b'")
		.Меньше(0);

	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("3.A", "3.a"), "Версия с 'A' равна версии с 'a'")
		.Равно(0);

	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("123.abc", "123abc"), "Версии с точкой и без равны")
		.Равно(0);

	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1.0", "1.0.1"), "Версия 1.0 меньше чем 1.0.1")
		.Меньше(0);

	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1.0", "10"), "Версия 1.0 меньше чем 10")
		.Меньше(0);
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1.....", "1..1"), "Версия 1..... меньше чем 1..1")
		.Меньше(0);
	
	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("", ""), "Пустые версии равны")
		.Равно(0);

	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("1", ""), "Первая версия больше пустой второй")
		.Больше(0);

	Ожидаем
		.Что(Модуль.СравнитьНомераВерсий("", "2"), "Пустая первая версия меньше второй")
		.Меньше(0);
