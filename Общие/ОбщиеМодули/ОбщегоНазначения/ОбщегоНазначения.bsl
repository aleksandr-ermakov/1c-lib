...
//									Если не указан, определяется по состоянию документа и метаданным.
//									Элементы:
...
Процедура УстановитьЗначенияРеквизитовОбъекта(
	...
	Если ТипЗнч(ОбменДанными) = Тип("Структура") Тогда
		...
		ОбменДаннымиЗаполняемый.Вставить("Загрузка",	 Ложь);
		ОбменДаннымиЗаполняемый.Вставить("Отправитель",	 Неопределено);
		ОбменДаннымиЗаполняемый.Вставить("Получатели",	 Новый Массив);
		ОбменДаннымиЗаполняемый.Вставить("ЗагруАвтоЗаполнениезка", Неопределено);
	...
	Если ЭтоДокументСсылка Тогда                 
		...
		ДокументПоддерживаетПроведение = Ссылка.Метаданные().Проведение = Метаданные.СвойстваОбъектов.Проведение.Разрешить;
		ДокументПроведенИлиПроводится = 
			ДокументПоддерживаетПроведение
			И (Объект.Проведен 
				Или ПараметрыЗаписиДокумента.Свойство("РежимЗаписи") 
					И ПараметрыЗаписиДокумента.РежимЗаписи = РежимЗаписиДокумента.Проведение
			);  
				
		Если ДокументПроведенИлиПроводится И Не Объект.ОбменДанными.Загрузка Тогда 
			
			РежимЗаписи = РежимЗаписиДокумента.Проведение;
			
			СвойстваОбъектов = Метаданные.СвойстваОбъектов;
			МетаданныеДокумента = Ссылка.Метаданные();
			ДоступноОперативноеПроведение = 
				МетаданныеДокумента.Проведение = СвойстваОбъектов.Проведение.Разрешить 
				И МетаданныеДокумента.ОперативноеПроведение = СвойстваОбъектов.ОперативноеПроведение.Разрешить;
			ПроводитьОперативно = Объект.Дата >= НачалоДня(ТекущаяДатаСеанса());
			Если ДоступноОперативноеПроведение И ПроводитьОперативно Тогда
		...
		ПараметрыЗаписи = Новый Структура("РежимЗаписи, РежимПроведения",	РежимЗаписи, РежимПроведения);
		
		Если ТипЗнч(ПараметрыЗаписиДокумента) = Тип("Структура") Тогда
			ЗаполнитьЗначенияСвойств(ПараметрыЗаписи, ПараметрыЗаписиДокумента)
		КонецЕсли;
...
// Под листьями понимаются узлы дерева значений, не имеющие дочерних узлов.
//
// Параметры:
//	Узел - ДеревоЗначений, СтрокаДереваЗначений - Узел, листья которого нужно получить.
//
// Возвращаемое значение:
//	Массив из СтрокаДереваЗначений - Массив листьев дерева значений.
//
Функция ЛистьяДереваЗначений(Узел) Экспорт
	
	Листья = Новый Массив;
	Ветви = Узел.Строки;
	Если ЗначениеЗаполнено(Ветви) Тогда
		Для Каждого Ветвь Из Ветви Цикл
			Для Каждого Лист Из ЛистьяДереваЗначений(Ветвь) Цикл
...
// Определяет размещение значений в строках дерева значений.
//
// Параметры:
//	Узел	 - ДеревоЗначений, СтрокаДереваЗначений - Дерево значений для поиска.
//	Поле	 - Строка - Имя колонки, в которой будет осуществляться поиск. 
//
// Возвращаемое значение:
//	Соответствие из КлючИЗначение - Элементы:
//		* Ключ		 - Произвольный - Значение поля.
//		* Значение	 - Массив из ДанныеФормыЭлементДерева - Найденные узлы.
//
Функция РазмещениеЗначенийПоляДереваЗначений(Дерево, Поле) Экспорт
	
	Размещение = Новый Соответствие;
	Узлы = УзлыДереваЗначений(Дерево);

	Для Каждого Узел Из Узлы Цикл
		Значение = Узел[Поле];
		РазмещениеЗначения = Размещение[Значение];
		Если РазмещениеЗначения = Неопределено Тогда
			РазмещениеЗначения = Новый Массив;
			Размещение.Вставить(Значение, РазмещениеЗначения);
		КонецЕсли;
		РазмещениеЗначения.Добавить(Узел);
	КонецЦикла;
	
	Возврат Размещение;

КонецФункции // РазмещениеЗначенийПоляДереваЗначений()

// Получает рекурсивно все элементы, подчиненные переданному узлу дерева значений.
//
// Параметры:
//  Узел - ДеревоЗначений, СтрокаДереваЗначений
//
// Возвращаемое значение:
//	 - Массив из СтрокаДереваЗначений
//
Функция УзлыДереваЗначений(Узел) Экспорт

	ВозвращаемоеЗначение = Новый Массив;
	Для каждого Ветвь Из Узел.Строки Цикл
		ВозвращаемоеЗначение.Добавить(Ветвь);
		Для каждого УзелВетви Из УзлыДереваЗначений(Ветвь) Цикл
...
// Проверяет, что расширение конфигурацииподключено и активно
//
// Параметры:
//  Расширение	 - РасширениеКонфигурации  - Расширение конфигурации.
//				 - УникальныйИдентификатор - Уникальный идентификатор расширения.
//				 - Строка - Имя расширения.
// 
// Возвращаемое значение:
//  Булево - Расширение подлключено и активно
//
Функция РасширениеАктивно(Знач Расширение) Экспорт
	
	Если ТипЗнч(Расширение) = Тип("Строка") Тогда
		Расширения = РасширенияКонфигурации.Получить(Новый Структура("Имя", Расширение));
		Если Не ЗначениеЗаполнено(Расширения) Тогда
			Возврат Ложь;
		КонецЕсли;
		Расширение = Расширения[0];
	КонецЕсли;
	Если ТипЗнч(Расширение) = Тип("УникальныйИдентификатор") Тогда
		Расширения = РасширенияКонфигурации.Получить(Новый Структура("УникальныйИдентификатор", Расширение));
		Если Не ЗначениеЗаполнено(Расширения) Тогда
			Возврат Ложь;
		КонецЕсли;
		Расширение = Расширения[0];
	...
	Если ТипЗнч(Расширение) <> Тип("РасширениеКонфигурации") Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Неверный тип параметра Расширение: %1'"), ТипЗнч(Расширение));
	КонецЕсли; 
	
	Возврат Расширение.Активно;

КонецФункции // РасширениеАктивно()

// Дополняет колонки таблицы или дерева значений колонками из другого источника.
//
// Параметры:
//  Приемник - ТаблицаЗначений, ДеревоЗначений - Таблица или дерево значений, в которые нужно добавить колонки.
//  Источник - ТаблицаЗначений, ДеревоЗначений - Таблица или дерево значений, из которых нужно добавить колонки
//  Колонки	 - Строка - Имена колонок, которые нужно добавить, через запятую. 
//			 - Массив из Строка - Имена колонок, которые нужно добавить.
//			 - Массив из КолонкаТаблицыЗначений - Колонки Источника, которые нужно добавить.
//			 - Массив из КолонкиДереваЗначений - Колонки Источника, которые нужно добавить.
//			Если не указан, добавляются все колонки из Источника, которых нет в Приемнике.
// 			Если указана колонка, которой нет в Источнике, вызывается исключение.
// 			Если указана колонка, которая уже есть в Приемнике, а колонка Приемника типизирована,
//			тип её значений дополняется до типа колонки Источника.
//			Если колонка Приемника типизирована, а колонки Источника нет, 
//			тип колонки Приемника дополняется типами значений Источника. 
//			Существующие квалификаторы типов колонок переносимых колонок в приемнике сбрасываются на значения по умолчанию.
//
Процедура ДополнитьКолонки(Знач Приемник, Знач Источник, Знач Колонки = Неопределено) Экспорт

	Если ТипЗнч(Источник) = Тип("ТаблицаЗначений")
		Или ТипЗнч(Источник) = Тип("ДеревоЗначений") Тогда
		КолонкиИсточника = Источник.Колонки;
	Иначе
		ВызватьИсключение "Параметр Источник: Ожидается тип ТаблицейЗначений или ДеревоЗначений.";
	КонецЕсли;

	Если ТипЗнч(Приемник) = Тип("ТаблицаЗначений")
		Или ТипЗнч(Приемник) = Тип("ДеревоЗначений") Тогда
		КолонкиПриемника = Приемник.Колонки;
	Иначе
		ВызватьИсключение "Параметр Приемник: Ожидается тип ТаблицейЗначений или ДеревоЗначений.";
	КонецЕсли;

	ИменаКолонок = Новый Массив;
	ОбщиеИменаКолонок = Новый Массив;
	Если Колонки = Неопределено Тогда
		Для каждого Колонка Из КолонкиИсточника Цикл
			Если КолонкиПриемника.Найти(Колонка.Имя) = Неопределено Тогда
				ИменаКолонок.Добавить(Колонка.Имя);
			Иначе
				ОбщиеИменаКолонок.Добавить(Колонка.Имя);
			КонецЕсли;
		КонецЦикла;
		...
		Для каждого Колонка Из Колонки Цикл
			Если ТипЗнч(Колонка) = Тип("Строка") Тогда
				Если ПустаяСтрока(Колонка) Тогда
					ВызватьИсключение "Параметр Колонки: Пустая строка в массиве.";
				КонецЕсли;
				Если КолонкиИсточника.Найти(Колонка) = Неопределено Тогда
					ВызватьИсключение "Параметр Колонки: Колонка '" + Колонка + "' не найдена в Источнике.";
				КонецЕсли;
				Если КолонкиПриемника.Найти(Колонка) = Неопределено Тогда
					ИменаКолонок.Добавить(Колонка);
				Иначе
					ОбщиеИменаКолонок.Добавить(Колонка.Имя);
				КонецЕсли;
			ИначеЕсли ТипЗнч(Колонка) = Тип("КолонкаТаблицыЗначений")
				Или ТипЗнч(Колонка) = Тип("КолонкаДереваЗначений") Тогда
				Если КолонкиИсточника.Индекс(Колонка) < 0 Тогда
					ВызватьИсключение "Параметр Колонки: Колонка '" + Колонка.Имя + "' не найдена в Источнике.";
				КонецЕсли;
				Если КолонкиПриемника.Найти(Колонка.Имя) = Неопределено Тогда
					ИменаКолонок.Добавить(Колонка.Имя);
				Иначе
				...
				КонецЕсли;
			Иначе
				ВызватьИсключение "Параметр Колонки: Ожидается тип Строка, КолонкаТаблицыЗначений или КолонкаДереваЗначений.";
			КонецЕсли;
		КонецЦикла;
	ИначеЕсли ТипЗнч(Колонки) = Тип("Строка") Тогда
		Колонки = СтрРазделить(Колонки, ", ", Ложь);
		Для каждого ИмяКолонки Из Колонки Цикл
			Если КолонкиИсточника.Найти(ИмяКолонки) = Неопределено Тогда
				ВызватьИсключение "Параметр Колонки: Колонка '" + ИмяКолонки + "' не найдена в Источнике.";
			КонецЕсли;
			Если КолонкиПриемника.Найти(ИмяКолонки) = Неопределено Тогда
				ИменаКолонок.Добавить(ИмяКолонки);
			Иначе
				ОбщиеИменаКолонок.Добавить(ИмяКолонки);
		КонецЕсли;
		КонецЦикла;
	Иначе
		ВызватьИсключение "Параметр Колонки: Ожидается тип Массив.";
	КонецЕсли;

	// Самое главное, что делает этот метод:
	Для каждого ИмяКолонки Из ИменаКолонок Цикл
		КолонкаИсточника = КолонкиИсточника[ИмяКолонки];
		КолонкиПриемника.Добавить(ИмяКолонки, 
	...
	// Решаем, какие типы значений переносим, а какие определяем по значениям Источника:
	КолонкиИсточникаДляОпределенияТипа = Новый Массив;
	КолонкиДляДополненияТипа = Новый Массив;
	ИменаКлонокГдеИзменяетсяТип = Новый Массив;
	Для каждого ИмяКолонки Из ОбщиеИменаКолонок Цикл
		КолонкаИсточника = КолонкиИсточника.Найти(ИмяКолонки);
		КолонкаПриемника = КолонкиПриемника.Найти(ИмяКолонки);
		ТипИсточника = КолонкаИсточника.ТипЗначения;
		ТипПриемника = КолонкаПриемника.ТипЗначения;
		Если ТипПриемника = Новый ОписаниеТипов() Тогда
			Продолжить;
		КонецЕсли;
		Если ТипИсточника = Новый ОписаниеТипов() Тогда
			// Колонка Источника не типизирована.
			// Дополняем тип Приемника до типа значений Источника.			
			КолонкиИсточникаДляОпределенияТипа.Добавить(ИмяКолонки);
		Иначе
			// Обе колонки типизированы.
			// Дополняем тип Приемника до типа Источника.
			КолонкиДляДополненияТипа.Добавить(ИмяКолонки);
		КонецЕсли;
		ИменаКлонокГдеИзменяетсяТип.Добавить(ИмяКолонки);
	КонецЦикла;

	// Определяем типы значений колонок Источника:
	ТипыЗначенийКолонокИсточника = Новый Структура;	// Ключ - ИмяКолонки, Значение - Массив из Тип
	Если ЗначениеЗаполнено(КолонкиИсточникаДляОпределенияТипа) Тогда

		СтрокиИсточника = Новый Массив;
		Если ТипЗнч(Источник) = Тип("ТаблицаЗначений") Тогда
			Для каждого СтрокаИсточника Из Источник Цикл
				СтрокиИсточника.Добавить(СтрокаИсточника);
			КонецЦикла;
		Иначе	// ДеревоЗначений
			// Рекурсивно обходим дерево значений, получая значения колонок Источника.
			Для каждого Узел Из Источник.Строки Цикл
				СтрокиИсточника.Добавить(Узел);
			КонецЦикла;
			Для каждого Узел Из СтрокиИсточника Цикл
				СтрокиИсточника.Добавить(Узел);
				Для каждого ПодчиненныйУзел Из Узел.Строки Цикл
					СтрокиИсточника.Добавить(ПодчиненныйУзел);
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;

		Для каждого ИмяКолонкиИсточника Из КолонкиИсточникаДляОпределенияТипа Цикл
			ТипыЗначенийКолонки = Новый Массив;
			Для каждого СтрокаИсточника Из СтрокиИсточника Цикл
				ТипЗначения = ТипЗнч(СтрокаИсточника[ИмяКолонкиИсточника]);
				Если ТипыЗначенийКолонки.Найти(ТипЗначения) = Неопределено Тогда
					ТипыЗначенийКолонки.Добавить(ТипЗначения);
				КонецЕсли;
			КонецЦикла;
			ТипыЗначенийКолонокИсточника.Вставить(ИмяКолонкиИсточника, ТипыЗначенийКолонки);
		КонецЦикла;

	КонецЕсли;

	// Сохраняем исходные значения колонок, где будет изменяться тип:
	ИменаКлонокГдеИзменяетсяТипСтрокой = СтрСоединить(ИменаКлонокГдеИзменяетсяТип, ", ");
	ЗначенияКолонокГдеИзменяетсяТип = Новый Соответствие;
	СтрокиПриемника = Новый Массив;
	Если ЗначениеЗаполнено(ИменаКлонокГдеИзменяетсяТип) Тогда
		
		Если ТипЗнч(Приемник) = Тип("ТаблицаЗначений") Тогда
			Для каждого СтрокаИсточника Из Приемник Цикл
				СтрокиПриемника.Добавить(СтрокаИсточника);
			КонецЦикла;
		Иначе	// ДеревоЗначений
			// Рекурсивно обходим дерево значений, получая значения колонок Источника.
			Для каждого Узел Из Приемник.Строки Цикл
				СтрокиПриемника.Добавить(Узел);
			КонецЦикла;
			Для каждого Узел Из СтрокиПриемника Цикл
				СтрокиПриемника.Добавить(Узел);
				Для каждого ПодчиненныйУзел Из Узел.Строки Цикл
					СтрокиПриемника.Добавить(ПодчиненныйУзел);
				КонецЦикла;
			КонецЦикла;
		КонецЕсли;

		Для каждого СтрокаПриемника Из СтрокиПриемника Цикл
			ЗначенияКолонок = Новый Структура(ИменаКлонокГдеИзменяетсяТипСтрокой);
			ЗаполнитьЗначенияСвойств(ЗначенияКолонок, СтрокаПриемника);
			ЗначенияКолонокГдеИзменяетсяТип.Вставить(СтрокаПриемника, ЗначенияКолонок);
		КонецЦикла;

	КонецЕсли;

	// Формируем новые описания типов колонок Приемника:
	НовыеТипыКолонокПриемника = Новый Структура;	// Ключ - ИмяКолонки, Значение - ОписаниеТипов
	Для каждого ИмяКолонки Из КолонкиИсточникаДляОпределенияТипа Цикл
		ИсходноеОписаниеТипов = КолонкиПриемника.Найти(ИмяКолонки).ТипЗначения;
		ТипыПриемника = ИсходноеОписаниеТипов.Типы();
		ДобавляемыеТипы = Новый Массив;
		ТипыИсточника = ТипыЗначенийКолонокИсточника[ИмяКолонки];
		Для каждого ТипИсточника Из ТипыИсточника Цикл
			Если ТипыПриемника.Найти(ТипИсточника) = Неопределено Тогда
				ДобавляемыеТипы.Добавить(ТипИсточника);
			КонецЕсли;
		КонецЦикла;
		КвалификаторыЧисла = ?(
			ТипыИсточника.Найти(Тип("Число")) <> Неопределено, 
			Новый КвалификаторыЧисла(), 
			ИсходноеОписаниеТипов.КвалификаторыЧисла
		);
		КвалификаторыСтроки = ?(
			ТипыИсточника.Найти(Тип("Строка")) <> Неопределено, 
			Новый КвалификаторыСтроки(), 
			ИсходноеОписаниеТипов.КвалификаторСтроки
		);
		КвалификаторыДаты = ?(
			ТипыИсточника.Найти(Тип("Дата")) <> Неопределено, 
			Новый КвалификаторыДаты(), 
			ИсходноеОписаниеТипов.КвалификаторыДаты
		);
		КвалификаторыДвоичныхДанных = ?(
			ТипыИсточника.Найти(Тип("ДвоичныеДанные")) <> Неопределено, 
			Новый КвалификаторыДвоичныхДанных(), 
			ИсходноеОписаниеТипов.КвалификаторыДвоичныхДанных
		);
		НовоеОписаниеТипов = Новый ОписаниеТипов(ИсходноеОписаниеТипов, ДобавляемыеТипы, ,
			КвалификаторыЧисла, КвалификаторыСтроки, КвалификаторыДаты, КвалификаторыДвоичныхДанных
		);
		НовыеТипыКолонокПриемника.Вставить(ИмяКолонки, НовоеОписаниеТипов);
	КонецЦикла;
	Для Каждого ИмяКолонки Из КолонкиДляДополненияТипа Цикл
		КолонкаИсточника = КолонкиИсточника[ИмяКолонки]; 
		КолонкаПриемника = КолонкиПриемника[ИмяКолонки];
		ТипКолонкиПриемника = КолонкаПриемника.ТипЗначения;
		Для каждого ТипИсточника Из КолонкаИсточника.ТипЗначения.Типы() Цикл
			Если Не ТипКолонкиПриемника.СодержитТип(ТипИсточника) Тогда
				ДобавляемыеТипы.Добавить(ТипИсточника);
			КонецЕсли;
		КонецЦикла;
		...
		Иначе
			КвалификаторыЧисла = ТипКолонкиПриемника.КвалификаторыЧисла;
		КонецЕсли;
		Если КолонкаИсточника.ТипЗначения.СодержитТип(Тип("Строка")) Тогда
			КвалификаторыСтроки = Новый КвалификаторыСтроки();
		Иначе
			КвалификаторыСтроки = ТипКолонкиПриемника.КвалификаторыСтроки;
		КонецЕсли;
		Если КолонкаИсточника.ТипЗначения.СодержитТип(Тип("Дата")) Тогда
			КвалификаторыДаты = Новый КвалификаторыДаты();
		Иначе
			КвалификаторыДаты = ТипКолонкиПриемника.КвалификаторыДаты;
		КонецЕсли;
		Если КолонкаИсточника.ТипЗначения.СодержитТип(Тип("ДвоичныеДанные")) Тогда
			КвалификаторыДвоичныхДанных = Новый КвалификаторыДвоичныхДанных();
		Иначе
			КвалификаторыДвоичныхДанных = ТипКолонкиПриемника.КвалификаторыДвоичныхДанных;
		КонецЕсли;
		НовоеОписаниеТипов = Новый ОписаниеТипов(ТипКолонкиПриемника, ДобавляемыеТипы, ,
			КвалификаторыЧисла, КвалификаторыСтроки, КвалификаторыДаты, КвалификаторыДвоичныхДанных
		);
		НовыеТипыКолонокПриемника.Вставить(ИмяКолонки, НовоеОписаниеТипов);
	КонецЦикла;

	// Меняем типы колонок Приемника:
	Для каждого ИмяКолонки Из ИменаКлонокГдеИзменяетсяТип Цикл
		
		ИсходнаяКолонка = КолонкиПриемника[ИмяКолонки];

		ВременноеИмяКолонки = ИмяКолонки + "_" + СтрЗаменить(Новый УникальныйИдентификатор(), "-", "");
		ТипЗначения = НовыеТипыКолонокПриемника[ИмяКолонки];
		ЗаголовокКолонки = ?(
				ЗначениеЗаполнено(КолонкиИсточника[ИмяКолонки].Заголовок), 
				КолонкиИсточника[ИмяКолонки].Заголовок, 
				ИсходнаяКолонка.Заголовок
		);
		ШиринаКолонки = Макс(КолонкиИсточника[ИмяКолонки].Ширина, ИсходнаяКолонка.Ширина);
		НоваяКолонка = КолонкиПриемника.Добавить(
			ВременноеИмяКолонки, 
			ТипЗначения,
			ЗаголовокКолонки,
			ШиринаКолонки
		);
		КолонкиПриемника.Сдвинуть(
			НоваяКолонка, 
			КолонкиПриемника.Индекс(ИсходнаяКолонка) - КолонкиПриемника.Индекс(НоваяКолонка)
		);
		КолонкиПриемника.Удалить(ИмяКолонки);
		КолонкиПриемника[ВременноеИмяКолонки].Имя = ИмяКолонки;

	КонецЦикла;
	Если ЗначениеЗаполнено(ИменаКлонокГдеИзменяетсяТип) Тогда
		// Возвращаем сохраненные значения колонок:
		Для каждого СтрокаЗначения Из ЗначенияКолонокГдеИзменяетсяТип Цикл
			ЗаполнитьЗначенияСвойств(СтрокаЗначения.Ключ, СтрокаЗначения.Значение);
		КонецЦикла;
	КонецЕсли;
...
// Проверяет проверяет представление ссылки на несуществующий объект.
//
// Параметры:
//	Строка - Строка
//
// Возвращаемое значение:
//	Булево - Истина, если передана строка Вида "<Объект не найден> (...)"
//
Функция ЭтоСтрокаОбъектНеНайден(Строка) Экспорт

	РегулярноеВыражение = "<.+>\s\(\d+:[0-9a-fA-F]{32}\)";	// <Объект не найден> (333:9cd528c5d2a186f011f0ae33ae32b36c)
	Возврат СтрПодобнаПоРегулярномуВыражению(Строка, РегулярноеВыражение, Истина, Ложь);   
	
КонецФункции // ЭтоСтрокаОбъектНеНайден()
