...
//
Процедура ЗаменитьВременнуюТаблицуСхемойЗапроса(ИсходнаяСхемаЗапроса, ЗаменяемыйЗапросСхемы, Знач ЗаменяющаяСхемаЗапроса, ПрефиксЗаменающихТаблиц = "", Знач СоответствиеИменПолей = Неопределено)

	Если Ложь Тогда ИсходнаяСхемаЗапроса = Новый СхемаЗапроса КонецЕсли;
	Если Ложь Тогда ЗаменяемыйЗапросСхемы = ИсходнаяСхемаЗапроса.ПакетЗапросов.Добавить() КонецЕсли;
	...
	Для каждого ПрисоединяемыйЗапрос Из СхемаЗапросаПрисоединяемая.ПакетЗапросов Цикл
		...
		Иначе	
			...
			ЗапросСхемыДубликатПриемника = ИсходнаяСхемаЗапроса.ПакетЗапросов.ДобавитьКопию(ИсходнаяСхемаЗапроса.ПакетЗапросов.Индекс(ЗапросСхемыПриемник));	// Используется для проверки колонок и пререноса атрибутов.
			...
			Для каждого ВыражениеИндексаДубликат Из ЗапросСхемыДубликатПриемника.Индекс Цикл
				Если ТипЗнч(ВыражениеИндексаДубликат.Выражение) = Тип("КолонкаСхемыЗапроса") Тогда
					ЗапросСхемыПриемник.Индекс.Добавить(ЗапросСхемыПриемник.Колонки.Найти(ВыражениеИндексаДубликат.Выражение.Псевдоним));
