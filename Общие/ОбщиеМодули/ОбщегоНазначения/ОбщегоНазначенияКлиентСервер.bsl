...
//
Процедура ЗаполнитьПометкиДереваВверх(Узел, ИмяПоля = "Пометка") Экспорт
	...
	Если Родитель <> Неопределено Тогда
		ЗаполнитьПометкиДереваВверх(Родитель, ИмяПоля);
	КонецЕсли;
...
//	ИмяПоля			 - Строка	 - Заполняемое поле.
//
Процедура УстановитьПометкиДереваВниз(Узел, Пометка, ИмяПоля = "Пометка") Экспорт
	
	Узел[ИмяПоля] = Булево(Пометка);
	
	Ветви = ВетвиДерева(Узел);
	Для Каждого Ветвь Из Ветви Цикл
		УстановитьПометкиДереваВниз(Ветвь, Пометка, ИмяПоля);
	КонецЦикла;
...
// Начиная с версии платформы 8.3.10 следует использовать метод глобального контекста ЧислоИзШестнадцатеричнойСтроки().
//
// Параметры:
...
//
Функция ЧислоИзШестнадцатеричнойСтроки(Число) Экспорт
	ЧислоСтрокой = ПреобразоватьСистемуСчисления(Число, 16, 10);
	Возврат Число(ЧислоСтрокой);
КонецФункции // ЧислоИзШестнадцатеричнойСтроки()
...
//
Функция ШестнадцатеричноеЧисло(Знач Число) Экспорт
