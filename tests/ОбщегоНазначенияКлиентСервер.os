...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("АгрегатКоллекции");
...
Процедура Агрегат() Экспорт
	...
	Результат = Модуль.Агрегат(Значения, АгрегатнаяФункция);
	Ожидаем.Что(Результат, "КоличествоРазличных").Равно(5);
...
Процедура АгрегатКоллекции() Экспорт
	
	ИмяМетода = "АгрегатКоллекции";
	...
	Параметры = ОбщегоНазначенияКлиентСервер.МассивЗначений(Новый ТаблицаЗначений);
	Ожидаем
		.Что(Модуль, "Исключение при неверном параметре")
		.Метод(ИмяМетода, Параметры)
		.ВыбрасываетИсключение("Ожидается значение");

	Коллекция = Новый ТаблицаЗначений;
	Коллекция.Колонки.Добавить("Поле");
	Коллекция.Добавить().Поле = 1;
	Коллекция.Добавить().Поле = 2;
	Коллекция.Добавить().Поле = 3;
	Коллекция.Добавить().Поле = Неопределено;
	АгрегатнаяФункция = "Среднее";
	Поле = "Поле";
	Результат = Модуль.АгрегатКоллекции(Коллекция, АгрегатнаяФункция, Поле);
	Ожидаем.Что(Результат).Равно(2);

КонецПроцедуры // АгрегатКоллекции()
