...
//			 - КоллекцияКолонокТаблицыЗначений, КоллекцияКолонокДереваЗначений - Коллекция исходных колонок.
//  Источник - ТаблицаЗначений, ДеревоЗначений - Таблица или дерево значений, из которых нужно добавить колонки
//			 - КоллекцияКолонокТаблицыЗначений, КоллекцияКолонокДереваЗначений - Коллекция добавляемых колонок.
...
// 			Если укзазана колонка, которой нет в Источнике, вызывается исключение.
// 			Если указана колонка, которая уже есть в Приемнике, она пропускается.
//
Процедура ДополнитьКолонки(Знач Приемник, Знач Источник, Знач Колонки = Неопределено) Экспорт
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("КоллекцияКолонокТаблицыЗначений") Тогда
		КолонкиИсточника = Источник;
	Иначе
		ВызватьИсключение "Параметр Источник: Ожидается тип ТаблицейЗначений или КоллекциейКолонокТаблицыЗначений.";
	КонецЕсли;

	Если ТипЗнч(Приемник) = Тип("ТаблицаЗначений") Тогда
		КолонкиПриемника = Приемник.Колонки;
	ИначеЕсли ТипЗнч(Источник) = Тип("КоллекцияКолонокТаблицыЗначений") Тогда
		КолонкиПриемника = Приемник;
	Иначе
		ВызватьИсключение "Параметр Приемник: Ожидается тип ТаблицейЗначений или КоллекциейКолонокТаблицыЗначений.";
	КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Колонки) = Тип("Строка") Тогда
		Колонки = СтрРазделить(Колонки, ",", Истина);
	...
	Для каждого ИмяКолонки Из ИменаКолонок Цикл
		КолонкиПриемника.Колонки.Добавить(ИмяКолонки, 
