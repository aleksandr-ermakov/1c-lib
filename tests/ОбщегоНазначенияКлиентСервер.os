...
Процедура РаспределитьДатыПоХронологии() Экспорт
	...
	Сообщение  = "Исключение на неожиданный параметр";
	ИсходныеДаты = Неопределено;
	Параметры = ОбщегоНазначенияКлиентСервер.МассивЗначений(ИсходныеДаты);
	Ожидаем
		.Что(Модуль, Сообщение)
		.Метод(ИмяМетода, Параметры)
		.ВыбрасываетИсключение();

	Сообщение  = "Исключение на число в массиве";
	...
	Сообщение  = "Пустой результат на пустой масив";
	ИсходныеДаты = Новый Массив;
	Результат = Модуль.РаспределитьДатыПоХронологии(ИсходныеДаты);
	Ожидаем
		.Что(Результат, Сообщение)
		.ИмеетТип("Массив")
		.Не_().Заполнено();

	Сообщение  = "Проверяем изначально отсортированные даты";
	ИсходныеДаты = Новый Массив;
	ИсходныеДаты.Добавить(ТекущаяДата() + 0);
	ИсходныеДаты.Добавить(ТекущаяДата() + 1);
	ИсходныеДаты.Добавить(ТекущаяДата() + 2);
	Результат = Модуль.РаспределитьДатыПоХронологии(ИсходныеДаты);
	Для Индекс = 0 По ИсходныеДаты.Количество() - 1 Цикл
		Ожидаем
			.Что(Результат[Индекс], Сообщение)
			.Равно(ИсходныеДаты[Индекс]);
	КонецЦикла;
	
	Сообщение  = "Простая перестановка одной даты";
	ИсходныеДаты = Новый Массив;
	ИсходныеДаты.Добавить(ТекущаяДата() + 0);
	ИсходныеДаты.Добавить(ТекущаяДата() + 2);
	ИсходныеДаты.Добавить(ТекущаяДата() + 1);
	...
	Для Индекс = 0 По ИсходныеДаты.Количество() - 1 Цикл
		Ожидаем
			.Что(Результат[0], Сообщение).Равно(ИсходныеДаты[0])
			.Что(Результат[1], Сообщение).Равно(ИсходныеДаты[2])
			.Что(Результат[2], Сообщение).Равно(ИсходныеДаты[1]);
	КонецЦикла;
