...
//  ВычистаемыйДатаОкончания - Дата	 - Граница вычитаемого периода.
// 
// Возвращаемое значение:
//  Массив из Структура - Один или несколкьо пераодов, получаенные вычитанием. Элементы:
//		* ДатаНачала	 - Дата - Граница результирующего перода. 
//		* ДатаОкончания	 - Дата - Граница результирующего перода. 
//
Функция ВычестьПериод(УменьшаемыйДатаНачала, УменьшаемыйДатаОкончания, ВычитаемыйДатаНачала, ВычитаемыйДатаОкончания) Экспорт 
	...
	Если ВычитаемыйДатаНачала > ВычитаемыйДатаОкончания Тогда
		ВызватьИсключение "Неверно указан вычитамый период.";	
	КонецЕсли;
...
//	Строка
//
Функция УбратьЭкранированиеСпецсимволов(Знач Строка, Спецсимволы, СимволЭкранирования = "\")
...
//	Строка
//
Функция ЭкранироватьСпецсимволы(Знач Строка, Спецсимволы, СимволЭкранирования = "\")
		...
		Если Экранировать Тогда
			...
			Если Символ = "$" И (СтрДлина(Строка) - НомерСимвола >= ДлинаКонструкцииПеременной) Тогда
				...
				Если НачалоКонструкции = НомерСимвола + 1 И НачалоКонструкции < КонецКонструкции Тогда
					...
					Если ИмяПеременнойВалидно(ИмяПеременной, Ложь) Тогда
						Экранировать = Ложь;
					КонецЕсли;				
				КонецЕсли;
