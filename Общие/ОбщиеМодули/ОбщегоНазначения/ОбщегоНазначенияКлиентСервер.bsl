...
Функция ЦветПоКоду(Знач КодЦвета) Экспорт
	...
	Если СтрДлина(КодЦвета) <> 6 Тогда
		ВызватьИсключение "Неверная длина кода цвета";
	КонецЕсли;
	...
	Для НомерСимвола = 1 По СтрДлина(КодЦвета) Цикл
		Если Не СтрНайти(Разряды, Сред(КодЦвета, НомерСимвола, 1)) Тогда
			ВызватьИсключение "Неверный формат кода цвета";
		КонецЕсли;
	КонецЦикла;
	...
	Цвет = Новый Цвет(КодЦветаЧислами[0], КодЦветаЧислами[1], КодЦветаЧислами[2]);	
...
//
// Параметры:
//	Цвет	 - Цвет		 - Исходный цвет. При передаче цвета "Авто" вызывается исключение.
...
Функция КодЦвета(Знач Цвет, Префикс = "#", Кратко = Ложь) Экспорт
	...
	Если Цвет.Вид = ВидЦвета.АвтоЦвет Тогда
		ВызватьИсключение "Невозможно определить код цвета ""Авто""";
	КонецЕсли;
	...
	Алфавит = "0123456789ABCDEF";
	База = СтрДлина(Алфавит);
	Числа = Новый Соответствие;
	Для Индекс = 0 По База - 1 Цикл
		Числа[Индекс] = Сред(Алфавит, Индекс + 1, 1);
	КонецЦикла;
