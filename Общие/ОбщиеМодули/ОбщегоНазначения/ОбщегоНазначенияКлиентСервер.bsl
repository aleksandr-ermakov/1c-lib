...
// Получает листья дерева.
// Под листьями понимаются узлы дерева, не имеющие дочерних узлов.
//
// Параметры:
...
// Определяет размещение значений в строках дерева.
//
// Параметры:
//	Узел	 - ДанныеФормыДерево, ДанныеФормыЭлементДерева	 - Дерево формы для поиска.
//			 - ДеревоЗначений, СтрокаДереваЗначений			 - Дерево значений для поиска.
...
//
Функция РазмещениеЗначенийПоляДерева(Дерево, Поле) Экспорт
	
	Размещение = Новый Соответствие;
	Узлы = УзлыДерева(Дерево);
...
КонецФункции // РазмещениеЗначенийПоляДерева()
...
// Заполняет поле дерева значением уровня узла в дереве.
// 
// Параметры:
//	Узел			 - ДанныеФормыДерево, ДанныеФормыЭлементДерева
//					 - ДеревоЗначений, СтрокадереваЗначений - Узел дерева
//	ИмяПоляУровень	 - Строка - Имя поля дерева, в которое будет помещен уровень узла.
//	Уровень			 - Число - Уровень узла. Если передается дерево, уровень присваивается корневым строкам.
//								Для дочерних узлов уровень увеличивается на 1.
//
Процедура ЗаполнитьУровниДерева(Узел, ИмяПоляУровень = "Уровень", Уровень = 0) Экспорт

	ЭтоДерево = ТипЗнч(Узел) = Тип("ДанныеФормыДерево");
	#Если Сервер Или ВнешнееСоединение Тогда // BSLLS:UnreachableCode-off
	ЭтоДерево = ЭтоДерево ИЛИ ТипЗнч(Узел) = Тип("ДеревоЗначений");
	#КонецЕсли

	Если ЭтоДерево Тогда   
	
		Для Каждого Строка Из ВетвиДерева(Узел) Цикл
			ЗаполнитьУровниДерева(Строка, ИмяПоляУровень, Уровень); 
		КонецЦикла;
		
	Иначе // Строки дерева
		
		Узел[ИмяПоляУровень] = Уровень;
		Для Каждого Ветвь Из ВетвиДерева(Узел) Цикл
			ЗаполнитьУровниДерева(Ветвь, ИмяПоляУровень, Уровень + 1);
		КонецЦикла;
...
Функция ЗаполненныеЗначенияМассива(Массив) Экспорт
	...
	Для Каждого Значение Из Массив Цикл
		Если ЗначениеЗаполнено(Значение) Тогда
			МассивЗаполненных.Добавить(Значение);
		КонецЕсли;
	КонецЦикла;
...
// Ищет дубликаты значений в коллекции.
//
// Параметры:
//  Коллекция 	- Массив, Структура, Соответствие, Произвольный - Коллекция. Если указана не универсальная коллекция, 
//						она должна поддерживать обращение к значению элемента посредством оператора [...] (по индексу).
//	ВключаяНезаполненные - Булево - Определяет, следует ли включать незаполненные значения.
//									Работает только с мутабельными значениями элементов коллекции.
// 
// Возвращаемое значение:
//	Соответствие - Элементы:
//		* Ключ		 - Значение элемента коллекции
//		* Значение	 - Массив из Число - Индексы элементов коллекции, содержащих указанное значение.
//
Функция ДубликатыЗначенийКоллекции(Знач Коллекция, ВключаяНезаполненные = Истина) Экспорт

	РазмещениеЗначений = Новый Соответствие;
	ДублирующиесяЗначения = Новый Массив;
	РазмещениеДубликатов = Новый Соответствие;
	...
	Если ТипЗнч(Коллекция) = Тип("Структура")
		...
		Для Каждого КлючЗначение Из Коллекция Цикл
			...
			РазмещениеЗначения = РазмещениеЗначений[Значение];
			Если РазмещениеЗначения = Неопределено Тогда
				
				РазмещениеЗначения = Новый Массив;
				РазмещениеЗначений[Значение] = РазмещениеЗначения;

			Иначе
				ДублирующиесяЗначения.Добавить(Значение);
			КонецЕсли;
			РазмещениеЗначения.Добавить(КлючЗначение.Ключ);

		КонецЦикла;

	Иначе

		Для Индекс = 0 По Коллекция.Количество() - 1 Цикл
			...
			Иначе
				ДублирующиесяЗначения.Добавить(Значение);
			КонецЕсли;
			РазмещениеЗначения.Добавить(Индекс);

		КонецЦикла;

	КонецЕсли;

	Для Каждого Значение Из ДублирующиесяЗначения Цикл
		РазмещениеДубликатов[Значение] = РазмещениеЗначений[Значение];
	КонецЦикла;

	Возврат РазмещениеДубликатов;

КонецФункции // ДубликатыЗначенийКоллекции()
...
// Получает значения указанного свойства элементов коллекции.
//	Например, все значения колонки из коллекции формы.
//
// Параметры:
//	Коллекция	 - Произвольный	 - Любая коллекция.
//	Свойство	 - Строка		 - Имя свойства элементов коллекции.
//
// Возвращаемое значение:
//	Массив из Произвольный - Значения свойства элементов коллекции.
//		Количество и порядок элементов массива соответствуют количеству и порядку элементов коллекции.
//		Если элемент коллекции не содержит указанного свойства, 
//		в соответствующем элементе массива будет Неопределено.
//
Функция ЗначенияСвойстваЭлементовКоллекции(Коллекция, Свойство) Экспорт

	ЗначенияСвойства = Новый Массив;

	#Если Сервер Тогда
	Если ТипЗнч(Коллекция) = Тип("ТаблицаЗначений") 
		И Коллекция.Колонки.Найти(Свойство) <> Неопределено Тогда
		Возврат Коллекция.ВыгрузитьКолонку(Свойство);
	КонецЕсли;
	#КонецЕсли
	
	Для Каждого ЭлементКоллекции Из Коллекция Цикл
		ЗначениеСвойства = Новый Структура(Свойство);
		ЗаполнитьЗначенияСвойств(ЗначениеСвойства, ЭлементКоллекции);
		ЗначенияСвойства.Добавить(ЗначениеСвойства[Свойство]);
	КонецЦикла;
	
	Возврат ЗначенияСвойства;
	
КонецФункции // ЗначенияСвойстваЭлементовКоллекции()
...
//					 Для такой проверки, поля элементов коллекции должны быть перечислимыми,
//					 например, СтрокаТаблицыЗначений 
...
Функция НайтиЭлементыКоллекции(Коллекция, Знач ПараметрыПоиска, Знач ПоляПоиска = Неопределено) Экспорт
	...
	Для Каждого ЭлементКоллекции Из Коллекция Цикл
		...
		Если ЭтоПоискПоОдномуЗначению И ЗначениеЗаполнено(ПоляПоиска) Тогда
			...
			СтруктураПроверки = Новый Структура(СтрСоединить(ПоляПоиска, ", "));
			ЗаполнитьЗначенияСвойств(СтруктураПроверки, ЭлементКоллекции);
			Для Каждого Поле Из ПоляПоиска Цикл
				Если СтруктураПроверки[Поле] = ИскомоеЗначение Тогда
...
//
// Возвращаемое значение:
//	Массив из Произвольный - Отобранные элементы в порядке следования в исходной коллекции.
//
...
Функция ОбратныйПорядок(ИсходнаяКоллекция) Экспорт
	...
	Для Каждого ТекущийЭлемент Из Элементы Цикл
		ЭлементыВОбратномПорядке[Индекс] = ТекущийЭлемент;
		Индекс = Индекс - 1;
	КонецЦикла;
...
// Разбивает коллекцию на порции указанного размера.
//
// Параметры:
//	Коллекция	 - Произвольный	 - Исходная коллекция. Должна поддерживать обход "Для Каждого...".
//	РазмерПорции - Число		 - Количество элементов в порции.
//
// Возвращаемое значение:
//	Массив из Массив - Порции элементов коллекции.
//
Функция ПорцииЭлементовКоллекции(Коллекция, Знач РазмерПорции) Экспорт
	
	РазмерПорции = Цел(РазмерПорции);

	Порции = Новый Массив;
	Порция = Новый Массив;
	Для каждого ЭлементКоллекции Из Коллекция Цикл
		Порция.Добавить(ЭлементКоллекции);
...
// Параметры:
//	Значение				 - Произвольный	 - Проверяемое значение.
//	ЗначениеЕслиНеЗаполнено	 - Произвольный	 - Значение, возвращаемое, если Значение не заполнено.
//
// Возвращаемое значение:
//	Произвольный - Результат проверки.
//
Функция ЕстьЗначение(Значение, ЗначениеЕслиНеЗаполнено) Экспорт

	Возврат ?(ЗначениеЗаполнено(Значение), Значение, ЗначениеЕслиНеЗаполнено);
