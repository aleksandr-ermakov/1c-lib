...
//
Функция НовыйПараметрыПоискаПоИндексуКоллекции(ИндексКоллекции) Экспорт

	ПараметрыПоиска = Новый Структура;
	Для каждого Поле Из ИндексКоллекции.Поля Цикл
		ПараметрыПоиска.Вставить(Поле);
	КонецЦикла;
...
// Параметры:
//	ИндексКоллекции - Структура - См. ПостроитьИндексКоллекции()
//	ПараметрыПоиска - Структура - см. НовыйПараметрыПоискаПоИндексуКоллекции()
...
Функция НайтиПоИндексуКоллекции(ИндексКоллекции, ПараметрыПоиска) Экспорт
	...
	Для Индекс = 0 По ИндексКоллекции.Поля.ВГРаница() Цикл
		...
		СлойПоЗначению = ТекущийСлой[Поле];
		Если СлойПоЗначению = Неопределено Тогда
			Прервать;
		КонецЕсли;
		ТекущийСлой = СлойПоЗначению;
...
//
// Параметры:
//	Коллекция	 - ТаблицаЗначений, ДанныеФормыКоллекция, Произвольный - Коллекция из элементов с именованными полями.
//	Данные		 - ТаблицаЗначений, Произвольный - Обновленные данные.
//	КлючевыеПоля - Строка - Имена ключевых полей, по которым сопоставляются существубщие и обновленные элементы.
...
//	Сортировать	 - Булево - Если Истина, элементы коллекции будут идти в порядке следования элементов Данных.
//							Если Ложь, новые элементы данных будут добавлены в конец Коллекции.
//
Процедура ОбновитьКоллекцию(
	...
	КлючевыеПоляСтруктура = Новый Структура(КлючевыеПоляСтрокой);

	ТипКоллекции = ТипЗнч(Коллекция);
	КоллекцияПоддерживаетМетодНайтиСтроки = ТипКоллекции = Тип("ДанныеФормыКоллекция")
		Или ТипКоллекции = Тип("ДанныеФормыСтруктураСКоллекцией");
	#Если Сервер Или ВнешнееСоединение Тогда
	КоллекцияПоддерживаетМетодНайтиСтроки = КоллекцияПоддерживаетМетодНайтиСтроки
		Или ТипКоллекции = Тип("ТаблицаЗначений")
		Или ТипКоллекции = Тип("КоллекцияСтрокДереваЗначений");
	#КонецЕсли

	#Область Удаление
	Если Удалять Тогда
		УдаляемыеЭлементы = Новый Массив;
		ИндексКоллекции = ПостроитьИндексКоллекции(Данные, КлючевыеПоля);
		ПараметрыПоиска = НовыйПараметрыПоискаПоИндексуКоллекции(ИндексКоллекции);
		Для каждого ЭлементКоллекции Из Коллекция Цикл
			ЗаполнитьЗначенияСвойств(ПараметрыПоиска, ЭлементКоллекции);
			ЭлементыДанных = НайтиПоИндексуКоллекции(ИндексКоллекции, ПараметрыПоиска);
			Если Не ЗначениеЗаполнено(ЭлементыДанных) Тогда
				УдаляемыеЭлементы.Добавить(ЭлементКоллекции);
			КонецЕсли;
		КонецЦикла;
		Для Каждого УдаляемыйЭлемент Из УдаляемыеЭлементы Цикл
			Попытка
				Коллекция.Удалить(УдаляемыйЭлемент);
				УдалосьУдалитьПоЗначению = Истина;
			Исключение
				УдалосьУдалитьПоЗначению = Ложь;
			КонецПопытки;
			Если Не УдалосьУдалитьПоЗначению Тогда
				Попытка
					Коллекция.Удалить(Коллекция.Индекс(УдаляемыйЭлемент));
				Исключение
					ВызватьИсключение "Не удалось удалить элемент коллекции по причине:
					|" + ОписаниеОшибки();
				КонецПопытки;
			КонецЕсли;
		КонецЦикла;
