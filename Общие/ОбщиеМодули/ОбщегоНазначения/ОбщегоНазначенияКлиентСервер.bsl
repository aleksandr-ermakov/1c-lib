...
Функция РаспределитьДатыПоХронологии(Знач ИсходныеДаты, Знач Период = "") Экспорт
	...
	Пока Истина Цикл
		...
		Если ЭтоОбходВперед И ТекущаяДата > ВерхняяГраницаДаты Тогда
			ДатыПоХронологии[Индекс] = ВерхняяГраницаДаты;
			МассивДатОтсортирован = Ложь;
			НадоРазвернутьОбход = Истина;
		КонецЕсли;
		Если Не ЭтоОбходВперед И ТекущаяДата < НижняяГраницаДаты Тогда
			ДатыПоХронологии[Индекс] = НижняяГраницаДаты;
			МассивДатОтсортирован = Ложь;
			НадоРазвернутьОбход = Истина;
		КонецЕсли;
