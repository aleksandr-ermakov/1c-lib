	...
	Если ПроверятьГраницыПериода Тогда
		...
		Для каждого НачалоПериодаДаты Из НачалоПериодаДат Цикл
			...
			Если МинИсходнаяДата > НачалоПериода И МаксИсходнаяДата < ОкончаниеПериода Тогда 
				...
				МаксГраницаДаты = Мин(МинИсходнаяДата + КоличествоШаговСменыДат * ШагСменыДат, ОкончаниеПериода);
				МинГраницаДаты = Макс(МаксГраницаДаты - КоличествоШаговСменыДат * ШагСменыДат, НачалоПериода);

			ИначеЕсли МинИсходнаяДата > НачалоПериода Тогда
				МинГраницаДаты = Мин(МинИсходнаяДата, ОкончаниеПериода - КоличествоШаговСменыДат * ШагСменыДат);
				МаксГраницаДаты = ОкончаниеПериода; 

			ИначеЕсли ОкончаниеПериода > МаксИсходнаяДата Тогда
				МинГраницаДаты = НачалоПериода;
				МаксГраницаДаты = Макс(МаксИсходнаяДата, НачалоПериода + КоличествоШаговСменыДат * ШагСменыДат); 
