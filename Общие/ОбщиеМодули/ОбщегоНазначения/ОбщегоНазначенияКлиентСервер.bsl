...
// Параметры:
//	Коллекция	 - Произвольный	 - Любая коллекция, 
//								поддерживающая обращение по индексу, методы Количество() и Сдвинуть(Индекс, Смещение).
...
Процедура СортироватьКоллекцию(Коллекция, Поля) Экспорт
	...
	ВыраженияСортировки = СтрРазделить(Поля, ",", Ложь);
	...
	Для каждого ВыражениеСортировки Из ВыраженияСортировки Цикл
		ВыраженияСортировкиОбратныйПорядок.Вставить(0, СокрЛП(ВыражениеСортировки));
	КонецЦикла;
	...
	СписокСортировки = Новый СписокЗначений;
	ЭлементыКоллекцииСписка = Новый Соответствие;
	ЭлементыСпискаКоллекции = Новый Соответствие;
	Для Каждого ЭлементКоллекции Из Коллекция Цикл
		ЭлементСписка = СписокСортировки.Добавить(Неопределено);
		ЭлементыКоллекцииСписка[ЭлементКоллекции] = ЭлементСписка;
		ЭлементыСпискаКоллекции[ЭлементСписка] = ЭлементКоллекции;
	КонецЦикла;

	Для каждого ВыражениеСортировки Из ВыраженияСортировкиОбратныйПорядок Цикл
		...
		Для каждого ЭлементКоллекции Из Коллекция Цикл
			ЭлементыКоллекцииСписка[ЭлементКоллекции].Значение = ЭлементКоллекции[ПолеСортировки];
		КонецЦикла;
		СписокСортировки.СортироватьПоЗначению(Направление);

		Если КоллекцияЭтоМассив Тогда
			
		Иначе
			Для каждого ЭлементСписка Из СписокСортировки Цикл
				ЭлементКоллекции = ЭлементыСпискаКоллекции[ЭлементСписка];
				Индекс = Коллекция.Индекс(ЭлементКоллекции);
				Сдвиг = СписокСортировки.Индекс(ЭлементСписка) - Индекс;
				Коллекция.Сдвинуть(Индекс, Сдвиг);
			КонецЦикла;
		КонецЕсли;
