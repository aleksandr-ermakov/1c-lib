...
Процедура ПоместитьТаблицуЗначенийВоВременнуюТаблицу(Таблица, Менеджер, Имя, Поля = "", Индексы = "")
	...
	ИндексПоля = 0;
	МаксИндексПоля = ИменаКолонок.ВГраница();
	Для каждого ИмяПоля Из ИменаКолонок Цикл
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + Символы.Таб + Псевдоним + "." + ИмяПоля; 
		Если ИндексПоля <> МаксИндексПоля Тогда
	...
	Если ЗначениеЗаполнено(Индексы) Тогда
		ТекстЗапроса = ТекстЗапроса + Символы.ПС + "ИНДЕКСИРОВАТЬ ПО";
		ИменаПолейИндекса = СтрРазделить(Индексы, ", ", Ложь);
		ИндексПоля = 0;
		МаксИндексПоля = ИменаПолейИндекса.ВГраница();
		Для каждого ИмяПоля Из ИменаПолейИндекса Цикл
			ТекстЗапроса = ТекстЗапроса + Символы.ПС + Символы.Таб + Псевдоним + "." + ИмяПоля; 
			Если ИндексПоля <> МаксИндексПоля Тогда
				ТекстЗапроса = ТекстЗапроса + ",";
			КонецЕсли;
