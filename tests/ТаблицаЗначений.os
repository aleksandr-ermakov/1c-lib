...
Функция ПолучитьСписокТестов(ЮнитТестирование) Экспорт
	...
	ВсеТесты.Добавить("ПроверитьУникальностьЗначенийТаблицы");
...
Процедура ПроверитьУникальностьЗначенийТаблицы() Экспорт

	Таблица = Новый ТаблицаЗначений;
	Таблица.Колонки.Добавить("Код", Новый ОписаниеТипов("Строка"));
	Таблица.Колонки.Добавить("Имя", Новый ОписаниеТипов("Строка"));
	Таблица.Добавить().Код = "A"; Таблица[0].Имя = "Иван";
	Таблица.Добавить().Код = "B"; Таблица[1].Имя = "Петр";
	Таблица.Добавить().Код = "A"; Таблица[2].Имя = "Сидор";
	Неуникальные = Неопределено;
	Результат = Модуль.ПроверитьУникальностьЗначенийТаблицы(Таблица, "Код", Неуникальные);
	Ожидаем.Что(Результат, "Есть неуникальные значения, ожидается Ложь").Равно(Ложь);
	Ожидаем.Что(Неуникальные, "Неуникальные значения должны быть массивом").ИмеетТип("Массив");
	Ожидаем.Что(Неуникальные.Количество(), "Массив неуникальных значений должен быть заполнен").Равно(1);
	Ожидаем.Что(Неуникальные[0].Код, "Первое неуникальное значение должно быть 'A'").Равно("A");
	
КонецПроцедуры // ПроверитьУникальностьЗначенийТаблицы()

//Файл = Новый Файл("..");
//Сообщить(Файл.ПолноеИмя);
//ВыполнитьОтдельныйТест(1);

ПолучитьСписокТестов(ЮнитТестирование);
ПроверитьУникальностьЗначенийТаблицы();