...
//								Возможные значения: "День", "Месяц", "Год".
...
Функция РаспределитьДатыПоХронологии(Знач ИсходныеДаты, Знач Период = "") Экспорт
	...
	Если ПроверятьГраницыПериода Тогда
		Для каждого ТекущаяДата Из ИсходныеДаты Цикл
			...
			КонецЕсли;
			НижниеГраницыДат[ТекущаяДата] = Макс(МинимальнаяДата, НижняяГраницаДаты);
			ВерхниеГраницыДат[ТекущаяДата] = Мин(МаксимальнаяДата, ВерхняяГраницаДаты);
		КонецЦикла;
	Иначе
		Для каждого ТекущаяДата Из ИсходныеДаты Цикл
	...
	Пока Истина Цикл
		...
		Если ЭтоПервыйЭлемент Тогда 
			ПредыдущаяДата = ?(ЭтоОбходВперед,
				НижниеГраницыДат[ИсходнаяДата] - ШагСменыДат,
				ВерхниеГраницыДат[ИсходнаяДата]) + ШагСменыДат;
		Иначе
		...
		Если Не ЭлементОтсортирован Тогда
			...
			НоваяДата = Макс(НоваяДата, НижняяГраницаДаты);
			НоваяДата = Мин(НоваяДата, ВерхняяГраницаДаты);
		...
		Если ЭтоПоследнийЭлемент Тогда
			...
			Иначе
				// Разворачиваем направление обхода
				НаправлениеОбхода = ?(ЭтоОбходВперед, 
					НаправлениеОбходаНазад,
					НаправлениеОбходаВперед);
				МассивДатОтсортирован = Истина;
			КонецЕсли;
		КонецЕсли;
